(
	...
	if [ -n "${PHP_CLI_ALWAYS_POPULATE_RAW_POST_DATA+1}" ]; then
		echo "always_populate_raw_post_data=${PHP_CLI_ALWAYS_POPULATE_RAW_POST_DATA}"
	...
	if [ -n "${PHP_CLI_APC_RFC1867+1}" ]; then
		echo "apc.rfc1867=${PHP_CLI_APC_RFC1867}"
	elif [ -n "${PHP_APC_RFC1867+1}" ]; then
		echo "apc.rfc1867=${PHP_APC_RFC1867}"
	fi
	if [ -n "${PHP_CLI_APC_RFC1867_FREQ+1}" ]; then
		echo "apc.rfc1867_freq=${PHP_CLI_APC_RFC1867_FREQ}"
	elif [ -n "${PHP_APC_RFC1867_FREQ+1}" ]; then
		echo "apc.rfc1867_freq=${PHP_APC_RFC1867_FREQ}"
	fi
	if [ -n "${PHP_CLI_APC_RFC1867_NAME+1}" ]; then
		echo "apc.rfc1867_name=${PHP_CLI_APC_RFC1867_NAME}"
	elif [ -n "${PHP_APC_RFC1867_NAME+1}" ]; then
		echo "apc.rfc1867_name=${PHP_APC_RFC1867_NAME}"
	fi
	if [ -n "${PHP_CLI_APC_RFC1867_PREFIX+1}" ]; then
		echo "apc.rfc1867_prefix=${PHP_CLI_APC_RFC1867_PREFIX}"
	elif [ -n "${PHP_APC_RFC1867_PREFIX+1}" ]; then
		echo "apc.rfc1867_prefix=${PHP_APC_RFC1867_PREFIX}"
	fi
	if [ -n "${PHP_CLI_APC_RFC1867_TTL+1}" ]; then
		echo "apc.rfc1867_ttl=${PHP_CLI_APC_RFC1867_TTL}"
	...
	if [ -n "${PHP_CLI_APC_WRITABLE+1}" ]; then
		echo "apc.writable=${PHP_CLI_APC_WRITABLE}"
	...
	if [ -n "${PHP_CLI_ASP_TAGS+1}" ]; then
		echo "asp_tags=${PHP_CLI_ASP_TAGS}"
	...
	if [ -n "${PHP_CLI_MEMCACHED_SESS_BINARY+1}" ]; then
		echo "memcached.sess_binary=${PHP_CLI_MEMCACHED_SESS_BINARY}"
	...
	if [ -n "${PHP_CLI_MEMCACHED_SESS_REMOVE_FAILED+1}" ]; then
		echo "memcached.sess_remove_failed=${PHP_CLI_MEMCACHED_SESS_REMOVE_FAILED}"
	...
	if [ -n "${PHP_CLI_MEMCACHED_USE_SASL+1}" ]; then
		echo "memcached.use_sasl=${PHP_CLI_MEMCACHED_USE_SASL}"
	...
	if [ -n "${PHP_CLI_MONGO_ALLOW_EMPTY_KEYS+1}" ]; then
		echo "mongo.allow_empty_keys=${PHP_CLI_MONGO_ALLOW_EMPTY_KEYS}"
	elif [ -n "${PHP_MONGO_ALLOW_EMPTY_KEYS+1}" ]; then
		echo "mongo.allow_empty_keys=${PHP_MONGO_ALLOW_EMPTY_KEYS}"
	fi
	if [ -n "${PHP_CLI_MONGO_CHUNK_SIZE+1}" ]; then
		echo "mongo.chunk_size=${PHP_CLI_MONGO_CHUNK_SIZE}"
	elif [ -n "${PHP_MONGO_CHUNK_SIZE+1}" ]; then
		echo "mongo.chunk_size=${PHP_MONGO_CHUNK_SIZE}"
	fi
	if [ -n "${PHP_CLI_MONGO_CMD+1}" ]; then
		echo "mongo.cmd=${PHP_CLI_MONGO_CMD}"
	elif [ -n "${PHP_MONGO_CMD+1}" ]; then
		echo "mongo.cmd=${PHP_MONGO_CMD}"
	fi
	if [ -n "${PHP_CLI_MONGO_DEFAULT_HOST+1}" ]; then
		echo "mongo.default_host=${PHP_CLI_MONGO_DEFAULT_HOST}"
	elif [ -n "${PHP_MONGO_DEFAULT_HOST+1}" ]; then
		echo "mongo.default_host=${PHP_MONGO_DEFAULT_HOST}"
	fi
	if [ -n "${PHP_CLI_MONGO_DEFAULT_PORT+1}" ]; then
		echo "mongo.default_port=${PHP_CLI_MONGO_DEFAULT_PORT}"
	elif [ -n "${PHP_MONGO_DEFAULT_PORT+1}" ]; then
		echo "mongo.default_port=${PHP_MONGO_DEFAULT_PORT}"
	fi
	if [ -n "${PHP_CLI_MONGO_IS_MASTER_INTERVAL+1}" ]; then
		echo "mongo.is_master_interval=${PHP_CLI_MONGO_IS_MASTER_INTERVAL}"
	elif [ -n "${PHP_MONGO_IS_MASTER_INTERVAL+1}" ]; then
		echo "mongo.is_master_interval=${PHP_MONGO_IS_MASTER_INTERVAL}"
	fi
	if [ -n "${PHP_CLI_MONGO_LONG_AS_OBJECT+1}" ]; then
		echo "mongo.long_as_object=${PHP_CLI_MONGO_LONG_AS_OBJECT}"
	elif [ -n "${PHP_MONGO_LONG_AS_OBJECT+1}" ]; then
		echo "mongo.long_as_object=${PHP_MONGO_LONG_AS_OBJECT}"
	fi
	if [ -n "${PHP_CLI_MONGO_NATIVE_LONG+1}" ]; then
		echo "mongo.native_long=${PHP_CLI_MONGO_NATIVE_LONG}"
	elif [ -n "${PHP_MONGO_NATIVE_LONG+1}" ]; then
		echo "mongo.native_long=${PHP_MONGO_NATIVE_LONG}"
	fi
	if [ -n "${PHP_CLI_MONGO_PING_INTERVAL+1}" ]; then
		echo "mongo.ping_interval=${PHP_CLI_MONGO_PING_INTERVAL}"
	...
	if [ -n "${PHP_CLI_MSSQL_ALLOW_PERSISTENT+1}" ]; then
		echo "mssql.allow_persistent=${PHP_CLI_MSSQL_ALLOW_PERSISTENT}"
	elif [ -n "${PHP_MSSQL_ALLOW_PERSISTENT+1}" ]; then
		echo "mssql.allow_persistent=${PHP_MSSQL_ALLOW_PERSISTENT}"
	fi
	if [ -n "${PHP_CLI_MSSQL_BATCHSIZE+1}" ]; then
		echo "mssql.batchsize=${PHP_CLI_MSSQL_BATCHSIZE}"
	elif [ -n "${PHP_MSSQL_BATCHSIZE+1}" ]; then
		echo "mssql.batchsize=${PHP_MSSQL_BATCHSIZE}"
	fi
	if [ -n "${PHP_CLI_MSSQL_CHARSET+1}" ]; then
		echo "mssql.charset=${PHP_CLI_MSSQL_CHARSET}"
	elif [ -n "${PHP_MSSQL_CHARSET+1}" ]; then
		echo "mssql.charset=${PHP_MSSQL_CHARSET}"
	fi
	if [ -n "${PHP_CLI_MSSQL_COMPATABILITY_MODE+1}" ]; then
		echo "mssql.compatability_mode=${PHP_CLI_MSSQL_COMPATABILITY_MODE}"
	elif [ -n "${PHP_MSSQL_COMPATABILITY_MODE+1}" ]; then
		echo "mssql.compatability_mode=${PHP_MSSQL_COMPATABILITY_MODE}"
	fi
	if [ -n "${PHP_CLI_MSSQL_COMPATIBILITY_MODE+1}" ]; then
		echo "mssql.compatibility_mode=${PHP_CLI_MSSQL_COMPATIBILITY_MODE}"
	elif [ -n "${PHP_MSSQL_COMPATIBILITY_MODE+1}" ]; then
		echo "mssql.compatibility_mode=${PHP_MSSQL_COMPATIBILITY_MODE}"
	fi
	if [ -n "${PHP_CLI_MSSQL_CONNECT_TIMEOUT+1}" ]; then
		echo "mssql.connect_timeout=${PHP_CLI_MSSQL_CONNECT_TIMEOUT}"
	elif [ -n "${PHP_MSSQL_CONNECT_TIMEOUT+1}" ]; then
		echo "mssql.connect_timeout=${PHP_MSSQL_CONNECT_TIMEOUT}"
	fi
	if [ -n "${PHP_CLI_MSSQL_DATETIMECONVERT+1}" ]; then
		echo "mssql.datetimeconvert=${PHP_CLI_MSSQL_DATETIMECONVERT}"
	elif [ -n "${PHP_MSSQL_DATETIMECONVERT+1}" ]; then
		echo "mssql.datetimeconvert=${PHP_MSSQL_DATETIMECONVERT}"
	fi
	if [ -n "${PHP_CLI_MSSQL_MAX_LINKS+1}" ]; then
		echo "mssql.max_links=${PHP_CLI_MSSQL_MAX_LINKS}"
	elif [ -n "${PHP_MSSQL_MAX_LINKS+1}" ]; then
		echo "mssql.max_links=${PHP_MSSQL_MAX_LINKS}"
	fi
	if [ -n "${PHP_CLI_MSSQL_MAX_PERSISTENT+1}" ]; then
		echo "mssql.max_persistent=${PHP_CLI_MSSQL_MAX_PERSISTENT}"
	elif [ -n "${PHP_MSSQL_MAX_PERSISTENT+1}" ]; then
		echo "mssql.max_persistent=${PHP_MSSQL_MAX_PERSISTENT}"
	fi
	if [ -n "${PHP_CLI_MSSQL_MAX_PROCS+1}" ]; then
		echo "mssql.max_procs=${PHP_CLI_MSSQL_MAX_PROCS}"
	elif [ -n "${PHP_MSSQL_MAX_PROCS+1}" ]; then
		echo "mssql.max_procs=${PHP_MSSQL_MAX_PROCS}"
	fi
	if [ -n "${PHP_CLI_MSSQL_MIN_ERROR_SEVERITY+1}" ]; then
		echo "mssql.min_error_severity=${PHP_CLI_MSSQL_MIN_ERROR_SEVERITY}"
	elif [ -n "${PHP_MSSQL_MIN_ERROR_SEVERITY+1}" ]; then
		echo "mssql.min_error_severity=${PHP_MSSQL_MIN_ERROR_SEVERITY}"
	fi
	if [ -n "${PHP_CLI_MSSQL_MIN_MESSAGE_SEVERITY+1}" ]; then
		echo "mssql.min_message_severity=${PHP_CLI_MSSQL_MIN_MESSAGE_SEVERITY}"
	elif [ -n "${PHP_MSSQL_MIN_MESSAGE_SEVERITY+1}" ]; then
		echo "mssql.min_message_severity=${PHP_MSSQL_MIN_MESSAGE_SEVERITY}"
	fi
	if [ -n "${PHP_CLI_MSSQL_SECURE_CONNECTION+1}" ]; then
		echo "mssql.secure_connection=${PHP_CLI_MSSQL_SECURE_CONNECTION}"
	elif [ -n "${PHP_MSSQL_SECURE_CONNECTION+1}" ]; then
		echo "mssql.secure_connection=${PHP_MSSQL_SECURE_CONNECTION}"
	fi
	if [ -n "${PHP_CLI_MSSQL_TEXTLIMIT+1}" ]; then
		echo "mssql.textlimit=${PHP_CLI_MSSQL_TEXTLIMIT}"
	elif [ -n "${PHP_MSSQL_TEXTLIMIT+1}" ]; then
		echo "mssql.textlimit=${PHP_MSSQL_TEXTLIMIT}"
	fi
	if [ -n "${PHP_CLI_MSSQL_TEXTSIZE+1}" ]; then
		echo "mssql.textsize=${PHP_CLI_MSSQL_TEXTSIZE}"
	elif [ -n "${PHP_MSSQL_TEXTSIZE+1}" ]; then
		echo "mssql.textsize=${PHP_MSSQL_TEXTSIZE}"
	fi
	if [ -n "${PHP_CLI_MSSQL_TIMEOUT+1}" ]; then
		echo "mssql.timeout=${PHP_CLI_MSSQL_TIMEOUT}"
	elif [ -n "${PHP_MSSQL_TIMEOUT+1}" ]; then
		echo "mssql.timeout=${PHP_MSSQL_TIMEOUT}"
	fi
	if [ -n "${PHP_CLI_MYSQL_ALLOW_LOCAL_INFILE+1}" ]; then
		echo "mysql.allow_local_infile=${PHP_CLI_MYSQL_ALLOW_LOCAL_INFILE}"
	elif [ -n "${PHP_MYSQL_ALLOW_LOCAL_INFILE+1}" ]; then
		echo "mysql.allow_local_infile=${PHP_MYSQL_ALLOW_LOCAL_INFILE}"
	fi
	if [ -n "${PHP_CLI_MYSQL_ALLOW_PERSISTENT+1}" ]; then
		echo "mysql.allow_persistent=${PHP_CLI_MYSQL_ALLOW_PERSISTENT}"
	elif [ -n "${PHP_MYSQL_ALLOW_PERSISTENT+1}" ]; then
		echo "mysql.allow_persistent=${PHP_MYSQL_ALLOW_PERSISTENT}"
	fi
	if [ -n "${PHP_CLI_MYSQL_CONNECT_TIMEOUT+1}" ]; then
		echo "mysql.connect_timeout=${PHP_CLI_MYSQL_CONNECT_TIMEOUT}"
	elif [ -n "${PHP_MYSQL_CONNECT_TIMEOUT+1}" ]; then
		echo "mysql.connect_timeout=${PHP_MYSQL_CONNECT_TIMEOUT}"
	fi
	if [ -n "${PHP_CLI_MYSQL_DEFAULT_HOST+1}" ]; then
		echo "mysql.default_host=${PHP_CLI_MYSQL_DEFAULT_HOST}"
	elif [ -n "${PHP_MYSQL_DEFAULT_HOST+1}" ]; then
		echo "mysql.default_host=${PHP_MYSQL_DEFAULT_HOST}"
	fi
	if [ -n "${PHP_CLI_MYSQL_DEFAULT_PASSWORD+1}" ]; then
		echo "mysql.default_password=${PHP_CLI_MYSQL_DEFAULT_PASSWORD}"
	elif [ -n "${PHP_MYSQL_DEFAULT_PASSWORD+1}" ]; then
		echo "mysql.default_password=${PHP_MYSQL_DEFAULT_PASSWORD}"
	fi
	if [ -n "${PHP_CLI_MYSQL_DEFAULT_PORT+1}" ]; then
		echo "mysql.default_port=${PHP_CLI_MYSQL_DEFAULT_PORT}"
	elif [ -n "${PHP_MYSQL_DEFAULT_PORT+1}" ]; then
		echo "mysql.default_port=${PHP_MYSQL_DEFAULT_PORT}"
	fi
	if [ -n "${PHP_CLI_MYSQL_DEFAULT_SOCKET+1}" ]; then
		echo "mysql.default_socket=${PHP_CLI_MYSQL_DEFAULT_SOCKET}"
	elif [ -n "${PHP_MYSQL_DEFAULT_SOCKET+1}" ]; then
		echo "mysql.default_socket=${PHP_MYSQL_DEFAULT_SOCKET}"
	fi
	if [ -n "${PHP_CLI_MYSQL_DEFAULT_USER+1}" ]; then
		echo "mysql.default_user=${PHP_CLI_MYSQL_DEFAULT_USER}"
	elif [ -n "${PHP_MYSQL_DEFAULT_USER+1}" ]; then
		echo "mysql.default_user=${PHP_MYSQL_DEFAULT_USER}"
	fi
	if [ -n "${PHP_CLI_MYSQL_MAX_LINKS+1}" ]; then
		echo "mysql.max_links=${PHP_CLI_MYSQL_MAX_LINKS}"
	elif [ -n "${PHP_MYSQL_MAX_LINKS+1}" ]; then
		echo "mysql.max_links=${PHP_MYSQL_MAX_LINKS}"
	fi
	if [ -n "${PHP_CLI_MYSQL_MAX_PERSISTENT+1}" ]; then
		echo "mysql.max_persistent=${PHP_CLI_MYSQL_MAX_PERSISTENT}"
	elif [ -n "${PHP_MYSQL_MAX_PERSISTENT+1}" ]; then
		echo "mysql.max_persistent=${PHP_MYSQL_MAX_PERSISTENT}"
	fi
	if [ -n "${PHP_CLI_MYSQL_TRACE_MODE+1}" ]; then
		echo "mysql.trace_mode=${PHP_CLI_MYSQL_TRACE_MODE}"
	...
	if [ -n "${PHP_CLI_MYSQLND_MS_COLLECT_STATISTICS+1}" ]; then
		echo "mysqlnd_ms.collect_statistics=${PHP_CLI_MYSQLND_MS_COLLECT_STATISTICS}"
	elif [ -n "${PHP_MYSQLND_MS_COLLECT_STATISTICS+1}" ]; then
		echo "mysqlnd_ms.collect_statistics=${PHP_MYSQLND_MS_COLLECT_STATISTICS}"
	fi
	if [ -n "${PHP_CLI_MYSQLND_MS_CONFIG_FILE+1}" ]; then
		echo "mysqlnd_ms.config_file=${PHP_CLI_MYSQLND_MS_CONFIG_FILE}"
	elif [ -n "${PHP_MYSQLND_MS_CONFIG_FILE+1}" ]; then
		echo "mysqlnd_ms.config_file=${PHP_MYSQLND_MS_CONFIG_FILE}"
	fi
	if [ -n "${PHP_CLI_MYSQLND_MS_DISABLE_RW_SPLIT+1}" ]; then
		echo "mysqlnd_ms.disable_rw_split=${PHP_CLI_MYSQLND_MS_DISABLE_RW_SPLIT}"
	elif [ -n "${PHP_MYSQLND_MS_DISABLE_RW_SPLIT+1}" ]; then
		echo "mysqlnd_ms.disable_rw_split=${PHP_MYSQLND_MS_DISABLE_RW_SPLIT}"
	fi
	if [ -n "${PHP_CLI_MYSQLND_MS_ENABLE+1}" ]; then
		echo "mysqlnd_ms.enable=${PHP_CLI_MYSQLND_MS_ENABLE}"
	elif [ -n "${PHP_MYSQLND_MS_ENABLE+1}" ]; then
		echo "mysqlnd_ms.enable=${PHP_MYSQLND_MS_ENABLE}"
	fi
	if [ -n "${PHP_CLI_MYSQLND_MS_FORCE_CONFIG_USAGE+1}" ]; then
		echo "mysqlnd_ms.force_config_usage=${PHP_CLI_MYSQLND_MS_FORCE_CONFIG_USAGE}"
	elif [ -n "${PHP_MYSQLND_MS_FORCE_CONFIG_USAGE+1}" ]; then
		echo "mysqlnd_ms.force_config_usage=${PHP_MYSQLND_MS_FORCE_CONFIG_USAGE}"
	fi
	if [ -n "${PHP_CLI_MYSQLND_MS_MULTI_MASTER+1}" ]; then
		echo "mysqlnd_ms.multi_master=${PHP_CLI_MYSQLND_MS_MULTI_MASTER}"
	...
	if [ -n "${PHP_CLI_OPCACHE_LOAD_COMMENTS+1}" ]; then
		echo "opcache.load_comments=${PHP_CLI_OPCACHE_LOAD_COMMENTS}"
	...
	if [ -n "${PHP_CLI_XCACHE_ADMIN_ENABLE_AUTH+1}" ]; then
		echo "xcache.admin.enable_auth=${PHP_CLI_XCACHE_ADMIN_ENABLE_AUTH}"
	elif [ -n "${PHP_XCACHE_ADMIN_ENABLE_AUTH+1}" ]; then
		echo "xcache.admin.enable_auth=${PHP_XCACHE_ADMIN_ENABLE_AUTH}"
	fi
	if [ -n "${PHP_CLI_XCACHE_ALLOCATOR+1}" ]; then
		echo "xcache.allocator=${PHP_CLI_XCACHE_ALLOCATOR}"
	elif [ -n "${PHP_XCACHE_ALLOCATOR+1}" ]; then
		echo "xcache.allocator=${PHP_XCACHE_ALLOCATOR}"
	fi
	if [ -n "${PHP_CLI_XCACHE_CACHER+1}" ]; then
		echo "xcache.cacher=${PHP_CLI_XCACHE_CACHER}"
	elif [ -n "${PHP_XCACHE_CACHER+1}" ]; then
		echo "xcache.cacher=${PHP_XCACHE_CACHER}"
	fi
	if [ -n "${PHP_CLI_XCACHE_COREDUMP_DIRECTORY+1}" ]; then
		echo "xcache.coredump_directory=${PHP_CLI_XCACHE_COREDUMP_DIRECTORY}"
	elif [ -n "${PHP_XCACHE_COREDUMP_DIRECTORY+1}" ]; then
		echo "xcache.coredump_directory=${PHP_XCACHE_COREDUMP_DIRECTORY}"
	fi
	if [ -n "${PHP_CLI_XCACHE_COUNT+1}" ]; then
		echo "xcache.count=${PHP_CLI_XCACHE_COUNT}"
	elif [ -n "${PHP_XCACHE_COUNT+1}" ]; then
		echo "xcache.count=${PHP_XCACHE_COUNT}"
	fi
	if [ -n "${PHP_CLI_XCACHE_COVERAGEDUMP_DIRECTORY+1}" ]; then
		echo "xcache.coveragedump_directory=${PHP_CLI_XCACHE_COVERAGEDUMP_DIRECTORY}"
	elif [ -n "${PHP_XCACHE_COVERAGEDUMP_DIRECTORY+1}" ]; then
		echo "xcache.coveragedump_directory=${PHP_XCACHE_COVERAGEDUMP_DIRECTORY}"
	fi
	if [ -n "${PHP_CLI_XCACHE_COVERAGER+1}" ]; then
		echo "xcache.coverager=${PHP_CLI_XCACHE_COVERAGER}"
	elif [ -n "${PHP_XCACHE_COVERAGER+1}" ]; then
		echo "xcache.coverager=${PHP_XCACHE_COVERAGER}"
	fi
	if [ -n "${PHP_CLI_XCACHE_COVERAGER_AUTOSTART+1}" ]; then
		echo "xcache.coverager_autostart=${PHP_CLI_XCACHE_COVERAGER_AUTOSTART}"
	elif [ -n "${PHP_XCACHE_COVERAGER_AUTOSTART+1}" ]; then
		echo "xcache.coverager_autostart=${PHP_XCACHE_COVERAGER_AUTOSTART}"
	fi
	if [ -n "${PHP_CLI_XCACHE_DISABLE_ON_CRASH+1}" ]; then
		echo "xcache.disable_on_crash=${PHP_CLI_XCACHE_DISABLE_ON_CRASH}"
	elif [ -n "${PHP_XCACHE_DISABLE_ON_CRASH+1}" ]; then
		echo "xcache.disable_on_crash=${PHP_XCACHE_DISABLE_ON_CRASH}"
	fi
	if [ -n "${PHP_CLI_XCACHE_EXPERIMENTAL+1}" ]; then
		echo "xcache.experimental=${PHP_CLI_XCACHE_EXPERIMENTAL}"
	elif [ -n "${PHP_XCACHE_EXPERIMENTAL+1}" ]; then
		echo "xcache.experimental=${PHP_XCACHE_EXPERIMENTAL}"
	fi
	if [ -n "${PHP_CLI_XCACHE_GC_INTERVAL+1}" ]; then
		echo "xcache.gc_interval=${PHP_CLI_XCACHE_GC_INTERVAL}"
	elif [ -n "${PHP_XCACHE_GC_INTERVAL+1}" ]; then
		echo "xcache.gc_interval=${PHP_XCACHE_GC_INTERVAL}"
	fi
	if [ -n "${PHP_CLI_XCACHE_MMAP_PATH+1}" ]; then
		echo "xcache.mmap_path=${PHP_CLI_XCACHE_MMAP_PATH}"
	elif [ -n "${PHP_XCACHE_MMAP_PATH+1}" ]; then
		echo "xcache.mmap_path=${PHP_XCACHE_MMAP_PATH}"
	fi
	if [ -n "${PHP_CLI_XCACHE_OPTIMIZER+1}" ]; then
		echo "xcache.optimizer=${PHP_CLI_XCACHE_OPTIMIZER}"
	elif [ -n "${PHP_XCACHE_OPTIMIZER+1}" ]; then
		echo "xcache.optimizer=${PHP_XCACHE_OPTIMIZER}"
	fi
	if [ -n "${PHP_CLI_XCACHE_READONLY_PROTECTION+1}" ]; then
		echo "xcache.readonly_protection=${PHP_CLI_XCACHE_READONLY_PROTECTION}"
	elif [ -n "${PHP_XCACHE_READONLY_PROTECTION+1}" ]; then
		echo "xcache.readonly_protection=${PHP_XCACHE_READONLY_PROTECTION}"
	fi
	if [ -n "${PHP_CLI_XCACHE_SHM_SCHEME+1}" ]; then
		echo "xcache.shm_scheme=${PHP_CLI_XCACHE_SHM_SCHEME}"
	elif [ -n "${PHP_XCACHE_SHM_SCHEME+1}" ]; then
		echo "xcache.shm_scheme=${PHP_XCACHE_SHM_SCHEME}"
	fi
	if [ -n "${PHP_CLI_XCACHE_SIZE+1}" ]; then
		echo "xcache.size=${PHP_CLI_XCACHE_SIZE}"
	elif [ -n "${PHP_XCACHE_SIZE+1}" ]; then
		echo "xcache.size=${PHP_XCACHE_SIZE}"
	fi
	if [ -n "${PHP_CLI_XCACHE_SLOTS+1}" ]; then
		echo "xcache.slots=${PHP_CLI_XCACHE_SLOTS}"
	elif [ -n "${PHP_XCACHE_SLOTS+1}" ]; then
		echo "xcache.slots=${PHP_XCACHE_SLOTS}"
	fi
	if [ -n "${PHP_CLI_XCACHE_STAT+1}" ]; then
		echo "xcache.stat=${PHP_CLI_XCACHE_STAT}"
	elif [ -n "${PHP_XCACHE_STAT+1}" ]; then
		echo "xcache.stat=${PHP_XCACHE_STAT}"
	fi
	if [ -n "${PHP_CLI_XCACHE_TEST+1}" ]; then
		echo "xcache.test=${PHP_CLI_XCACHE_TEST}"
	elif [ -n "${PHP_XCACHE_TEST+1}" ]; then
		echo "xcache.test=${PHP_XCACHE_TEST}"
	fi
	if [ -n "${PHP_CLI_XCACHE_TTL+1}" ]; then
		echo "xcache.ttl=${PHP_CLI_XCACHE_TTL}"
	elif [ -n "${PHP_XCACHE_TTL+1}" ]; then
		echo "xcache.ttl=${PHP_XCACHE_TTL}"
	fi
	if [ -n "${PHP_CLI_XCACHE_VAR_ALLOCATOR+1}" ]; then
		echo "xcache.var_allocator=${PHP_CLI_XCACHE_VAR_ALLOCATOR}"
	elif [ -n "${PHP_XCACHE_VAR_ALLOCATOR+1}" ]; then
		echo "xcache.var_allocator=${PHP_XCACHE_VAR_ALLOCATOR}"
	fi
	if [ -n "${PHP_CLI_XCACHE_VAR_COUNT+1}" ]; then
		echo "xcache.var_count=${PHP_CLI_XCACHE_VAR_COUNT}"
	elif [ -n "${PHP_XCACHE_VAR_COUNT+1}" ]; then
		echo "xcache.var_count=${PHP_XCACHE_VAR_COUNT}"
	fi
	if [ -n "${PHP_CLI_XCACHE_VAR_GC_INTERVAL+1}" ]; then
		echo "xcache.var_gc_interval=${PHP_CLI_XCACHE_VAR_GC_INTERVAL}"
	elif [ -n "${PHP_XCACHE_VAR_GC_INTERVAL+1}" ]; then
		echo "xcache.var_gc_interval=${PHP_XCACHE_VAR_GC_INTERVAL}"
	fi
	if [ -n "${PHP_CLI_XCACHE_VAR_MAXTTL+1}" ]; then
		echo "xcache.var_maxttl=${PHP_CLI_XCACHE_VAR_MAXTTL}"
	elif [ -n "${PHP_XCACHE_VAR_MAXTTL+1}" ]; then
		echo "xcache.var_maxttl=${PHP_XCACHE_VAR_MAXTTL}"
	fi
	if [ -n "${PHP_CLI_XCACHE_VAR_NAMESPACE+1}" ]; then
		echo "xcache.var_namespace=${PHP_CLI_XCACHE_VAR_NAMESPACE}"
	elif [ -n "${PHP_XCACHE_VAR_NAMESPACE+1}" ]; then
		echo "xcache.var_namespace=${PHP_XCACHE_VAR_NAMESPACE}"
	fi
	if [ -n "${PHP_CLI_XCACHE_VAR_NAMESPACE_MODE+1}" ]; then
		echo "xcache.var_namespace_mode=${PHP_CLI_XCACHE_VAR_NAMESPACE_MODE}"
	elif [ -n "${PHP_XCACHE_VAR_NAMESPACE_MODE+1}" ]; then
		echo "xcache.var_namespace_mode=${PHP_XCACHE_VAR_NAMESPACE_MODE}"
	fi
	if [ -n "${PHP_CLI_XCACHE_VAR_SIZE+1}" ]; then
		echo "xcache.var_size=${PHP_CLI_XCACHE_VAR_SIZE}"
	elif [ -n "${PHP_XCACHE_VAR_SIZE+1}" ]; then
		echo "xcache.var_size=${PHP_XCACHE_VAR_SIZE}"
	fi
	if [ -n "${PHP_CLI_XCACHE_VAR_SLOTS+1}" ]; then
		echo "xcache.var_slots=${PHP_CLI_XCACHE_VAR_SLOTS}"
	elif [ -n "${PHP_XCACHE_VAR_SLOTS+1}" ]; then
		echo "xcache.var_slots=${PHP_XCACHE_VAR_SLOTS}"
	fi
	if [ -n "${PHP_CLI_XCACHE_VAR_TTL+1}" ]; then
		echo "xcache.var_ttl=${PHP_CLI_XCACHE_VAR_TTL}"
	...
	if [ -n "${PHP_CLI_XDEBUG_EXTENDED_INFO+1}" ]; then
		echo "xdebug.extended_info=${PHP_CLI_XDEBUG_EXTENDED_INFO}"
	...
	if [ -n "${PHP_CLI_XSL_SECURITY_PREFS+1}" ]; then
		echo "xsl.security_prefs=${PHP_CLI_XSL_SECURITY_PREFS}"
...
) | su -p -s /bin/sh -c 'cat > /etc/php/5.6/cli/conf.d/99-custom.ini'
