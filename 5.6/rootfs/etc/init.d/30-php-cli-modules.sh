...
if [ \( "${PHP_MODULES_ALL:-1}" != 0 -a -z "${PHP_CLI_MODULES_ALL}" \) -o \( "${PHP_CLI_MODULES_ALL}" != 0 -a -n "${PHP_CLI_MODULES_ALL}" \) ]; then
	...
	if [ \( "${PHP_MODULE_GEOIP:-1}" = 0 -a -z "${PHP_CLI_MODULE_GEOIP}" \) -o "${PHP_CLI_MODULE_GEOIP:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_INTERBASE:-1}" = 0 -a -z "${PHP_CLI_MODULE_INTERBASE}" \) -o "${PHP_CLI_MODULE_INTERBASE:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_JSON:-1}" = 0 -a -z "${PHP_CLI_MODULE_JSON}" \) -o "${PHP_CLI_MODULE_JSON:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_LUA:-1}" = 0 -a -z "${PHP_CLI_MODULE_LUA}" \) -o "${PHP_CLI_MODULE_LUA:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_MONGO:-1}" = 0 -a -z "${PHP_CLI_MODULE_MONGO}" \) -o "${PHP_CLI_MODULE_MONGO:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_MSSQL:-1}" = 0 -a -z "${PHP_CLI_MODULE_MSSQL}" \) -o "${PHP_CLI_MODULE_MSSQL:-1}" = 0 ]; then
		[ -h "$CONF/20-mssql.ini" ] && suexec rm -f "$CONF/20-mssql.ini"
	else
		[ -e "$CONF/20-mssql.ini" ] || suexec ln -sf "$MODS/mssql.ini" "$CONF/20-mssql.ini"
	fi
	if [ \( "${PHP_MODULE_MYSQL:-1}" = 0 -a -z "${PHP_CLI_MODULE_MYSQL}" \) -o "${PHP_CLI_MODULE_MYSQL:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_MYSQLND_MS:-1}" = 0 -a -z "${PHP_CLI_MODULE_MYSQLND_MS}" \) -o "${PHP_CLI_MODULE_MYSQLND_MS:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_PROPRO:-1}" = 0 -a -z "${PHP_CLI_MODULE_PROPRO}" \) -o "${PHP_CLI_MODULE_PROPRO:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_RADIUS:-1}" = 0 -a -z "${PHP_CLI_MODULE_RADIUS}" \) -o "${PHP_CLI_MODULE_RADIUS:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_RECODE:-1}" = 0 -a -z "${PHP_CLI_MODULE_RECODE}" \) -o "${PHP_CLI_MODULE_RECODE:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_WDDX:-1}" = 0 -a -z "${PHP_CLI_MODULE_WDDX}" \) -o "${PHP_CLI_MODULE_WDDX:-1}" = 0 ]; then
		[ -h "$CONF/20-wddx.ini" ] && suexec rm -f "$CONF/20-wddx.ini"
	else
		[ -e "$CONF/20-wddx.ini" ] || suexec ln -sf "$MODS/wddx.ini" "$CONF/20-wddx.ini"
	fi
	if [ \( "${PHP_MODULE_XCACHE:-1}" = 0 -a -z "${PHP_CLI_MODULE_XCACHE}" \) -o "${PHP_CLI_MODULE_XCACHE:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_GEOIP:-0}" != 0 -a -z "${PHP_CLI_MODULE_GEOIP}" \) -o "${PHP_CLI_MODULE_GEOIP:-0}" != 0 ]; then
		[ -e "$CONF/20-geoip.ini" ] || suexec ln -sf "$MODS/geoip.ini" "$CONF/20-geoip.ini"
	else
		[ -h "$CONF/20-geoip.ini" ] && suexec rm -f $CONF/20-geoip.ini
	fi
	...
	if [ \( "${PHP_MODULE_INTERBASE:-0}" != 0 -a -z "${PHP_CLI_MODULE_INTERBASE}" \) -o "${PHP_CLI_MODULE_INTERBASE:-0}" != 0 ]; then
		[ -e "$CONF/20-interbase.ini" ] || suexec ln -sf "$MODS/interbase.ini" "$CONF/20-interbase.ini"
	else
		[ -h "$CONF/20-interbase.ini" ] && suexec rm -f $CONF/20-interbase.ini
	fi
	...
	if [ \( "${PHP_MODULE_JSON:-0}" != 0 -a -z "${PHP_CLI_MODULE_JSON}" \) -o "${PHP_CLI_MODULE_JSON:-0}" != 0 ]; then
		[ -e "$CONF/20-json.ini" ] || suexec ln -sf "$MODS/json.ini" "$CONF/20-json.ini"
	else
		[ -h "$CONF/20-json.ini" ] && suexec rm -f $CONF/20-json.ini
	fi
	...
	if [ \( "${PHP_MODULE_LUA:-0}" != 0 -a -z "${PHP_CLI_MODULE_LUA}" \) -o "${PHP_CLI_MODULE_LUA:-0}" != 0 ]; then
		[ -e "$CONF/20-lua.ini" ] || suexec ln -sf "$MODS/lua.ini" "$CONF/20-lua.ini"
	else
		[ -h "$CONF/20-lua.ini" ] && suexec rm -f $CONF/20-lua.ini
	fi
	...
	if [ \( "${PHP_MODULE_MONGO:-0}" != 0 -a -z "${PHP_CLI_MODULE_MONGO}" \) -o "${PHP_CLI_MODULE_MONGO:-0}" != 0 ]; then
		[ -e "$CONF/20-mongo.ini" ] || suexec ln -sf "$MODS/mongo.ini" "$CONF/20-mongo.ini"
	else
		[ -h "$CONF/20-mongo.ini" ] && suexec rm -f $CONF/20-mongo.ini
	fi
	...
	if [ \( "${PHP_MODULE_MSSQL:-0}" != 0 -a -z "${PHP_CLI_MODULE_MSSQL}" \) -o "${PHP_CLI_MODULE_MSSQL:-0}" != 0 ]; then
		[ -e "$CONF/20-mssql.ini" ] || suexec ln -sf "$MODS/mssql.ini" "$CONF/20-mssql.ini"
	else
		[ -h "$CONF/20-mssql.ini" ] && suexec rm -f $CONF/20-mssql.ini
	fi
	if [ \( "${PHP_MODULE_MYSQL:-0}" != 0 -a -z "${PHP_CLI_MODULE_MYSQL}" \) -o "${PHP_CLI_MODULE_MYSQL:-0}" != 0 ]; then
		[ -e "$CONF/20-mysql.ini" ] || suexec ln -sf "$MODS/mysql.ini" "$CONF/20-mysql.ini"
	else
		[ -h "$CONF/20-mysql.ini" ] && suexec rm -f $CONF/20-mysql.ini
	fi
	...
	if [ \( "${PHP_MODULE_MYSQLND_MS:-0}" != 0 -a -z "${PHP_CLI_MODULE_MYSQLND_MS}" \) -o "${PHP_CLI_MODULE_MYSQLND_MS:-0}" != 0 ]; then
		[ -e "$CONF/30-mysqlnd_ms.ini" ] || suexec ln -sf "$MODS/mysqlnd_ms.ini" "$CONF/30-mysqlnd_ms.ini"
	else
		[ -h "$CONF/30-mysqlnd_ms.ini" ] && suexec rm -f $CONF/30-mysqlnd_ms.ini
	fi
	...
	if [ \( "${PHP_MODULE_PROPRO:-0}" != 0 -a -z "${PHP_CLI_MODULE_PROPRO}" \) -o "${PHP_CLI_MODULE_PROPRO:-0}" != 0 ]; then
		[ -e "$CONF/20-propro.ini" ] || suexec ln -sf "$MODS/propro.ini" "$CONF/20-propro.ini"
	else
		[ -h "$CONF/20-propro.ini" ] && suexec rm -f $CONF/20-propro.ini
	fi
	...
	if [ \( "${PHP_MODULE_RADIUS:-0}" != 0 -a -z "${PHP_CLI_MODULE_RADIUS}" \) -o "${PHP_CLI_MODULE_RADIUS:-0}" != 0 ]; then
		[ -e "$CONF/20-radius.ini" ] || suexec ln -sf "$MODS/radius.ini" "$CONF/20-radius.ini"
	else
		[ -h "$CONF/20-radius.ini" ] && suexec rm -f $CONF/20-radius.ini
	fi
	...
	if [ \( "${PHP_MODULE_RECODE:-0}" != 0 -a -z "${PHP_CLI_MODULE_RECODE}" \) -o "${PHP_CLI_MODULE_RECODE:-0}" != 0 ]; then
		[ -e "$CONF/20-recode.ini" ] || suexec ln -sf "$MODS/recode.ini" "$CONF/20-recode.ini"
	else
		[ -h "$CONF/20-recode.ini" ] && suexec rm -f $CONF/20-recode.ini
	fi
	...
	if [ \( "${PHP_MODULE_WDDX:-0}" != 0 -a -z "${PHP_CLI_MODULE_WDDX}" \) -o "${PHP_CLI_MODULE_WDDX:-0}" != 0 ]; then
		[ -e "$CONF/20-wddx.ini" ] || suexec ln -sf "$MODS/wddx.ini" "$CONF/20-wddx.ini"
	else
		[ -h "$CONF/20-wddx.ini" ] && suexec rm -f $CONF/20-wddx.ini
	fi
	if [ \( "${PHP_MODULE_XCACHE:-0}" != 0 -a -z "${PHP_CLI_MODULE_XCACHE}" \) -o "${PHP_CLI_MODULE_XCACHE:-0}" != 0 ]; then
		[ -e "$CONF/20-xcache.ini" ] || suexec ln -sf "$MODS/xcache.ini" "$CONF/20-xcache.ini"
	else
		[ -h "$CONF/20-xcache.ini" ] && suexec rm -f $CONF/20-xcache.ini
	fi
