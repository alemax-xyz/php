...
foreach ($modules as $module => $meta) {
    printf(
        '	[ \( "${PHP_MODULE_%1$s:-1}" = 0 -a -z "${PHP_FPM_MODULE_%1$s}" \) -o "${PHP_FPM_MODULE_%1$s:-1}" = 0 ] && rm -f %3$s || ln -sf %2$s %3$s' . PHP_EOL,
        $module,
        $meta['file'],
        $meta['link']
    );
...
foreach ($modules as $module => $meta) {
    printf(
        '	[ \( "${PHP_MODULE_%1$s:-0}" != 0 -a -z "${PHP_FPM_MODULE_%1$s}" \) -o "${PHP_FPM_MODULE_%1$s:-0}" != 0 ] && ln -sf %2$s %3$s || rm -f %3$s' . PHP_EOL,
        $module,
        $meta['file'],
        $meta['link']
    );
