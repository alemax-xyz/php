...
(
	...
	if [ -n "${PHP_CLI_BLACKFIRE_APM_BROWSER_KEY+1}" ]; then
		echo "blackfire.apm_browser_key=${PHP_CLI_BLACKFIRE_APM_BROWSER_KEY}"
	elif [ -n "${PHP_BLACKFIRE_APM_BROWSER_KEY+1}" ]; then
		echo "blackfire.apm_browser_key=${PHP_BLACKFIRE_APM_BROWSER_KEY}"
	fi
	if [ -n "${PHP_CLI_BLACKFIRE_APM_CONNECT_AT_STARTUP+1}" ]; then
		echo "blackfire.apm_connect_at_startup=${PHP_CLI_BLACKFIRE_APM_CONNECT_AT_STARTUP}"
	...
	if [ -n "${PHP_CLI_BLACKFIRE_APM_ENABLE_AUTOMATIC_BROWSER_PROBE+1}" ]; then
		echo "blackfire.apm_enable_automatic_browser_probe=${PHP_CLI_BLACKFIRE_APM_ENABLE_AUTOMATIC_BROWSER_PROBE}"
	...
	if [ -n "${PHP_CLI_BLACKFIRE_APM_ENABLED+1}" ]; then
		echo "blackfire.apm_enabled=${PHP_CLI_BLACKFIRE_APM_ENABLED}"
	elif [ -n "${PHP_BLACKFIRE_APM_ENABLED+1}" ]; then
		echo "blackfire.apm_enabled=${PHP_BLACKFIRE_APM_ENABLED}"
	fi
	if [ -n "${PHP_CLI_BLACKFIRE_APM_LOCK_DURATION+1}" ]; then
		echo "blackfire.apm_lock_duration=${PHP_CLI_BLACKFIRE_APM_LOCK_DURATION}"
	elif [ -n "${PHP_BLACKFIRE_APM_LOCK_DURATION+1}" ]; then
		echo "blackfire.apm_lock_duration=${PHP_BLACKFIRE_APM_LOCK_DURATION}"
	fi
	if [ -n "${PHP_CLI_BLACKFIRE_DISABLE_FEATURES+1}" ]; then
		echo "blackfire.disable_features=${PHP_CLI_BLACKFIRE_DISABLE_FEATURES}"
...
) > /etc/php/7.1/cli/conf.d/99-custom.ini
