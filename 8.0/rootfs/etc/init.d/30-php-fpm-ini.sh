(
	...
	if [ -n "${PHP_FPM_LOG_ERRORS_MAX_LEN+1}" ]; then
		echo "log_errors_max_len=${PHP_FPM_LOG_ERRORS_MAX_LEN}"
	...
	if [ -n "${PHP_FPM_MYSQLND_FETCH_DATA_COPY+1}" ]; then
		echo "mysqlnd.fetch_data_copy=${PHP_FPM_MYSQLND_FETCH_DATA_COPY}"
...
) | su -p -s /bin/sh -c 'cat > /etc/php/8.0/fpm/conf.d/99-custom.ini'
