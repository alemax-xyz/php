...
if [ \( "${PHP_MODULES_ALL:-1}" != 0 -a -z "${PHP_CLI_MODULES_ALL}" \) -o \( "${PHP_CLI_MODULES_ALL}" != 0 -a -n "${PHP_CLI_MODULES_ALL}" \) ]; then
	...
	if [ \( "${PHP_MODULE_APCU_BC:-1}" = 0 -a -z "${PHP_CLI_MODULE_APCU_BC}" \) -o "${PHP_CLI_MODULE_APCU_BC:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_PINBA:-1}" = 0 -a -z "${PHP_CLI_MODULE_PINBA}" \) -o "${PHP_CLI_MODULE_PINBA:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_SODIUM:-1}" = 0 -a -z "${PHP_CLI_MODULE_SODIUM}" \) -o "${PHP_CLI_MODULE_SODIUM:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_TIDEWAYS:-1}" = 0 -a -z "${PHP_CLI_MODULE_TIDEWAYS}" \) -o "${PHP_CLI_MODULE_TIDEWAYS:-1}" = 0 ]; then
	...
	if [ \( "${PHP_MODULE_APCU_BC:-0}" != 0 -a -z "${PHP_CLI_MODULE_APCU_BC}" \) -o "${PHP_CLI_MODULE_APCU_BC:-0}" != 0 ]; then
		[ -e "$CONF/25-apcu_bc.ini" ] || suexec ln -sf "$MODS/apcu_bc.ini" "$CONF/25-apcu_bc.ini"
	else
		[ -h "$CONF/25-apcu_bc.ini" ] && suexec rm -f $CONF/25-apcu_bc.ini
	fi
	...
	if [ \( "${PHP_MODULE_PINBA:-0}" != 0 -a -z "${PHP_CLI_MODULE_PINBA}" \) -o "${PHP_CLI_MODULE_PINBA:-0}" != 0 ]; then
		[ -e "$CONF/20-pinba.ini" ] || suexec ln -sf "$MODS/pinba.ini" "$CONF/20-pinba.ini"
	else
		[ -h "$CONF/20-pinba.ini" ] && suexec rm -f $CONF/20-pinba.ini
	fi
	...
	if [ \( "${PHP_MODULE_SODIUM:-0}" != 0 -a -z "${PHP_CLI_MODULE_SODIUM}" \) -o "${PHP_CLI_MODULE_SODIUM:-0}" != 0 ]; then
		[ -e "$CONF/20-sodium.ini" ] || suexec ln -sf "$MODS/sodium.ini" "$CONF/20-sodium.ini"
	else
		[ -h "$CONF/20-sodium.ini" ] && suexec rm -f $CONF/20-sodium.ini
	fi
	...
	if [ \( "${PHP_MODULE_TIDEWAYS:-0}" != 0 -a -z "${PHP_CLI_MODULE_TIDEWAYS}" \) -o "${PHP_CLI_MODULE_TIDEWAYS:-0}" != 0 ]; then
		[ -e "$CONF/20-tideways.ini" ] || suexec ln -sf "$MODS/tideways.ini" "$CONF/20-tideways.ini"
	else
		[ -h "$CONF/20-tideways.ini" ] && suexec rm -f $CONF/20-tideways.ini
	fi
