(
	...
	if [ -n "${PHP_FPM_OPCACHE_CONSISTENCY_CHECKS+1}" ]; then
		echo "opcache.consistency_checks=${PHP_FPM_OPCACHE_CONSISTENCY_CHECKS}"
	...
	if [ -n "${PHP_FPM_TIDEWAYS_XHPROF_CLOCK_USE_RDTSC+1}" ]; then
		echo "tideways_xhprof.clock_use_rdtsc=${PHP_FPM_TIDEWAYS_XHPROF_CLOCK_USE_RDTSC}"
...
) | su -p -s /bin/sh -c 'cat > /etc/php/8.2/fpm/conf.d/99-custom.ini'
