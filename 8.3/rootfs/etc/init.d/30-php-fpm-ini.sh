(
	if [ -n "${PHP_FPM_ALLOW_URL_FOPEN+1}" ]; then
		echo "allow_url_fopen=${PHP_FPM_ALLOW_URL_FOPEN}"
	elif [ -n "${PHP_ALLOW_URL_FOPEN+1}" ]; then
		echo "allow_url_fopen=${PHP_ALLOW_URL_FOPEN}"
	fi
	if [ -n "${PHP_FPM_ALLOW_URL_INCLUDE+1}" ]; then
		echo "allow_url_include=${PHP_FPM_ALLOW_URL_INCLUDE}"
	...
	if [ -n "${PHP_FPM_AMQP_CACERT+1}" ]; then
		echo "amqp.cacert=${PHP_FPM_AMQP_CACERT}"
	elif [ -n "${PHP_AMQP_CACERT+1}" ]; then
		echo "amqp.cacert=${PHP_AMQP_CACERT}"
	fi
	if [ -n "${PHP_FPM_AMQP_CERT+1}" ]; then
		echo "amqp.cert=${PHP_FPM_AMQP_CERT}"
	elif [ -n "${PHP_AMQP_CERT+1}" ]; then
		echo "amqp.cert=${PHP_AMQP_CERT}"
	fi
	if [ -n "${PHP_FPM_AMQP_CHANNEL_MAX+1}" ]; then
		echo "amqp.channel_max=${PHP_FPM_AMQP_CHANNEL_MAX}"
	elif [ -n "${PHP_AMQP_CHANNEL_MAX+1}" ]; then
		echo "amqp.channel_max=${PHP_AMQP_CHANNEL_MAX}"
	fi
	if [ -n "${PHP_FPM_AMQP_CONNECT_TIMEOUT+1}" ]; then
		echo "amqp.connect_timeout=${PHP_FPM_AMQP_CONNECT_TIMEOUT}"
	elif [ -n "${PHP_AMQP_CONNECT_TIMEOUT+1}" ]; then
		echo "amqp.connect_timeout=${PHP_AMQP_CONNECT_TIMEOUT}"
	fi
	if [ -n "${PHP_FPM_AMQP_DESERIALIZATION_DEPTH+1}" ]; then
		echo "amqp.deserialization_depth=${PHP_FPM_AMQP_DESERIALIZATION_DEPTH}"
	elif [ -n "${PHP_AMQP_DESERIALIZATION_DEPTH+1}" ]; then
		echo "amqp.deserialization_depth=${PHP_AMQP_DESERIALIZATION_DEPTH}"
	fi
	if [ -n "${PHP_FPM_AMQP_FRAME_MAX+1}" ]; then
		echo "amqp.frame_max=${PHP_FPM_AMQP_FRAME_MAX}"
	elif [ -n "${PHP_AMQP_FRAME_MAX+1}" ]; then
		echo "amqp.frame_max=${PHP_AMQP_FRAME_MAX}"
	fi
	if [ -n "${PHP_FPM_AMQP_GLOBAL_PREFETCH_COUNT+1}" ]; then
		echo "amqp.global_prefetch_count=${PHP_FPM_AMQP_GLOBAL_PREFETCH_COUNT}"
	elif [ -n "${PHP_AMQP_GLOBAL_PREFETCH_COUNT+1}" ]; then
		echo "amqp.global_prefetch_count=${PHP_AMQP_GLOBAL_PREFETCH_COUNT}"
	fi
	if [ -n "${PHP_FPM_AMQP_GLOBAL_PREFETCH_SIZE+1}" ]; then
		echo "amqp.global_prefetch_size=${PHP_FPM_AMQP_GLOBAL_PREFETCH_SIZE}"
	elif [ -n "${PHP_AMQP_GLOBAL_PREFETCH_SIZE+1}" ]; then
		echo "amqp.global_prefetch_size=${PHP_AMQP_GLOBAL_PREFETCH_SIZE}"
	fi
	if [ -n "${PHP_FPM_AMQP_HEARTBEAT+1}" ]; then
		echo "amqp.heartbeat=${PHP_FPM_AMQP_HEARTBEAT}"
	elif [ -n "${PHP_AMQP_HEARTBEAT+1}" ]; then
		echo "amqp.heartbeat=${PHP_AMQP_HEARTBEAT}"
	fi
	if [ -n "${PHP_FPM_AMQP_HOST+1}" ]; then
		echo "amqp.host=${PHP_FPM_AMQP_HOST}"
	elif [ -n "${PHP_AMQP_HOST+1}" ]; then
		echo "amqp.host=${PHP_AMQP_HOST}"
	fi
	if [ -n "${PHP_FPM_AMQP_KEY+1}" ]; then
		echo "amqp.key=${PHP_FPM_AMQP_KEY}"
	elif [ -n "${PHP_AMQP_KEY+1}" ]; then
		echo "amqp.key=${PHP_AMQP_KEY}"
	fi
	if [ -n "${PHP_FPM_AMQP_LOGIN+1}" ]; then
		echo "amqp.login=${PHP_FPM_AMQP_LOGIN}"
	elif [ -n "${PHP_AMQP_LOGIN+1}" ]; then
		echo "amqp.login=${PHP_AMQP_LOGIN}"
	fi
	if [ -n "${PHP_FPM_AMQP_PASSWORD+1}" ]; then
		echo "amqp.password=${PHP_FPM_AMQP_PASSWORD}"
	elif [ -n "${PHP_AMQP_PASSWORD+1}" ]; then
		echo "amqp.password=${PHP_AMQP_PASSWORD}"
	fi
	if [ -n "${PHP_FPM_AMQP_PORT+1}" ]; then
		echo "amqp.port=${PHP_FPM_AMQP_PORT}"
	elif [ -n "${PHP_AMQP_PORT+1}" ]; then
		echo "amqp.port=${PHP_AMQP_PORT}"
	fi
	if [ -n "${PHP_FPM_AMQP_PREFETCH_COUNT+1}" ]; then
		echo "amqp.prefetch_count=${PHP_FPM_AMQP_PREFETCH_COUNT}"
	elif [ -n "${PHP_AMQP_PREFETCH_COUNT+1}" ]; then
		echo "amqp.prefetch_count=${PHP_AMQP_PREFETCH_COUNT}"
	fi
	if [ -n "${PHP_FPM_AMQP_PREFETCH_SIZE+1}" ]; then
		echo "amqp.prefetch_size=${PHP_FPM_AMQP_PREFETCH_SIZE}"
	elif [ -n "${PHP_AMQP_PREFETCH_SIZE+1}" ]; then
		echo "amqp.prefetch_size=${PHP_AMQP_PREFETCH_SIZE}"
	fi
	if [ -n "${PHP_FPM_AMQP_READ_TIMEOUT+1}" ]; then
		echo "amqp.read_timeout=${PHP_FPM_AMQP_READ_TIMEOUT}"
	elif [ -n "${PHP_AMQP_READ_TIMEOUT+1}" ]; then
		echo "amqp.read_timeout=${PHP_AMQP_READ_TIMEOUT}"
	fi
	if [ -n "${PHP_FPM_AMQP_RPC_TIMEOUT+1}" ]; then
		echo "amqp.rpc_timeout=${PHP_FPM_AMQP_RPC_TIMEOUT}"
	elif [ -n "${PHP_AMQP_RPC_TIMEOUT+1}" ]; then
		echo "amqp.rpc_timeout=${PHP_AMQP_RPC_TIMEOUT}"
	fi
	if [ -n "${PHP_FPM_AMQP_SASL_METHOD+1}" ]; then
		echo "amqp.sasl_method=${PHP_FPM_AMQP_SASL_METHOD}"
	elif [ -n "${PHP_AMQP_SASL_METHOD+1}" ]; then
		echo "amqp.sasl_method=${PHP_AMQP_SASL_METHOD}"
	fi
	if [ -n "${PHP_FPM_AMQP_SERIALIZATION_DEPTH+1}" ]; then
		echo "amqp.serialization_depth=${PHP_FPM_AMQP_SERIALIZATION_DEPTH}"
	elif [ -n "${PHP_AMQP_SERIALIZATION_DEPTH+1}" ]; then
		echo "amqp.serialization_depth=${PHP_AMQP_SERIALIZATION_DEPTH}"
	fi
	if [ -n "${PHP_FPM_AMQP_TIMEOUT+1}" ]; then
		echo "amqp.timeout=${PHP_FPM_AMQP_TIMEOUT}"
	elif [ -n "${PHP_AMQP_TIMEOUT+1}" ]; then
		echo "amqp.timeout=${PHP_AMQP_TIMEOUT}"
	fi
	if [ -n "${PHP_FPM_AMQP_VERIFY+1}" ]; then
		echo "amqp.verify=${PHP_FPM_AMQP_VERIFY}"
	elif [ -n "${PHP_AMQP_VERIFY+1}" ]; then
		echo "amqp.verify=${PHP_AMQP_VERIFY}"
	fi
	if [ -n "${PHP_FPM_AMQP_VHOST+1}" ]; then
		echo "amqp.vhost=${PHP_FPM_AMQP_VHOST}"
	elif [ -n "${PHP_AMQP_VHOST+1}" ]; then
		echo "amqp.vhost=${PHP_AMQP_VHOST}"
	fi
	if [ -n "${PHP_FPM_AMQP_WRITE_TIMEOUT+1}" ]; then
		echo "amqp.write_timeout=${PHP_FPM_AMQP_WRITE_TIMEOUT}"
	elif [ -n "${PHP_AMQP_WRITE_TIMEOUT+1}" ]; then
		echo "amqp.write_timeout=${PHP_AMQP_WRITE_TIMEOUT}"
	fi
	if [ -n "${PHP_FPM_APC_COREDUMP_UNMAP+1}" ]; then
		echo "apc.coredump_unmap=${PHP_FPM_APC_COREDUMP_UNMAP}"
	elif [ -n "${PHP_APC_COREDUMP_UNMAP+1}" ]; then
		echo "apc.coredump_unmap=${PHP_APC_COREDUMP_UNMAP}"
	fi
	if [ -n "${PHP_FPM_APC_ENABLE_CLI+1}" ]; then
		echo "apc.enable_cli=${PHP_FPM_APC_ENABLE_CLI}"
	elif [ -n "${PHP_APC_ENABLE_CLI+1}" ]; then
		echo "apc.enable_cli=${PHP_APC_ENABLE_CLI}"
	fi
	if [ -n "${PHP_FPM_APC_ENABLED+1}" ]; then
		echo "apc.enabled=${PHP_FPM_APC_ENABLED}"
	elif [ -n "${PHP_APC_ENABLED+1}" ]; then
		echo "apc.enabled=${PHP_APC_ENABLED}"
	fi
	if [ -n "${PHP_FPM_APC_ENTRIES_HINT+1}" ]; then
		echo "apc.entries_hint=${PHP_FPM_APC_ENTRIES_HINT}"
	elif [ -n "${PHP_APC_ENTRIES_HINT+1}" ]; then
		echo "apc.entries_hint=${PHP_APC_ENTRIES_HINT}"
	fi
	if [ -n "${PHP_FPM_APC_GC_TTL+1}" ]; then
		echo "apc.gc_ttl=${PHP_FPM_APC_GC_TTL}"
	elif [ -n "${PHP_APC_GC_TTL+1}" ]; then
		echo "apc.gc_ttl=${PHP_APC_GC_TTL}"
	fi
	if [ -n "${PHP_FPM_APC_MMAP_FILE_MASK+1}" ]; then
		echo "apc.mmap_file_mask=${PHP_FPM_APC_MMAP_FILE_MASK}"
	elif [ -n "${PHP_APC_MMAP_FILE_MASK+1}" ]; then
		echo "apc.mmap_file_mask=${PHP_APC_MMAP_FILE_MASK}"
	fi
	if [ -n "${PHP_FPM_APC_MMAP_HUGEPAGE_SIZE+1}" ]; then
		echo "apc.mmap_hugepage_size=${PHP_FPM_APC_MMAP_HUGEPAGE_SIZE}"
	elif [ -n "${PHP_APC_MMAP_HUGEPAGE_SIZE+1}" ]; then
		echo "apc.mmap_hugepage_size=${PHP_APC_MMAP_HUGEPAGE_SIZE}"
	fi
	if [ -n "${PHP_FPM_APC_PRELOAD_PATH+1}" ]; then
		echo "apc.preload_path=${PHP_FPM_APC_PRELOAD_PATH}"
	...
	if [ -n "${PHP_FPM_APC_SHM_SIZE+1}" ]; then
		echo "apc.shm_size=${PHP_FPM_APC_SHM_SIZE}"
	elif [ -n "${PHP_APC_SHM_SIZE+1}" ]; then
		echo "apc.shm_size=${PHP_APC_SHM_SIZE}"
	fi
	if [ -n "${PHP_FPM_APC_SLAM_DEFENSE+1}" ]; then
		echo "apc.slam_defense=${PHP_FPM_APC_SLAM_DEFENSE}"
	elif [ -n "${PHP_APC_SLAM_DEFENSE+1}" ]; then
		echo "apc.slam_defense=${PHP_APC_SLAM_DEFENSE}"
	fi
	if [ -n "${PHP_FPM_APC_SMART+1}" ]; then
		echo "apc.smart=${PHP_FPM_APC_SMART}"
	elif [ -n "${PHP_APC_SMART+1}" ]; then
		echo "apc.smart=${PHP_APC_SMART}"
	fi
	if [ -n "${PHP_FPM_APC_TTL+1}" ]; then
		echo "apc.ttl=${PHP_FPM_APC_TTL}"
	elif [ -n "${PHP_APC_TTL+1}" ]; then
		echo "apc.ttl=${PHP_APC_TTL}"
	fi
	if [ -n "${PHP_FPM_APC_USE_REQUEST_TIME+1}" ]; then
		echo "apc.use_request_time=${PHP_FPM_APC_USE_REQUEST_TIME}"
	...
	if [ -n "${PHP_FPM_ARG_SEPARATOR_OUTPUT+1}" ]; then
	...
	if [ -n "${PHP_FPM_ASSERT_BAIL+1}" ]; then
		echo "assert.bail=${PHP_FPM_ASSERT_BAIL}"
	elif [ -n "${PHP_ASSERT_BAIL+1}" ]; then
		echo "assert.bail=${PHP_ASSERT_BAIL}"
	fi
	if [ -n "${PHP_FPM_ASSERT_CALLBACK+1}" ]; then
		echo "assert.callback=${PHP_FPM_ASSERT_CALLBACK}"
	...
	if [ -n "${PHP_FPM_AUTO_APPEND_FILE+1}" ]; then
		echo "auto_append_file=${PHP_FPM_AUTO_APPEND_FILE}"
	elif [ -n "${PHP_AUTO_APPEND_FILE+1}" ]; then
		echo "auto_append_file=${PHP_AUTO_APPEND_FILE}"
	fi
	if [ -n "${PHP_FPM_AUTO_DETECT_LINE_ENDINGS+1}" ]; then
		echo "auto_detect_line_endings=${PHP_FPM_AUTO_DETECT_LINE_ENDINGS}"
	elif [ -n "${PHP_AUTO_DETECT_LINE_ENDINGS+1}" ]; then
		echo "auto_detect_line_endings=${PHP_AUTO_DETECT_LINE_ENDINGS}"
	fi
	if [ -n "${PHP_FPM_AUTO_GLOBALS_JIT+1}" ]; then
		echo "auto_globals_jit=${PHP_FPM_AUTO_GLOBALS_JIT}"
	elif [ -n "${PHP_AUTO_GLOBALS_JIT+1}" ]; then
		echo "auto_globals_jit=${PHP_AUTO_GLOBALS_JIT}"
	fi
	if [ -n "${PHP_FPM_AUTO_PREPEND_FILE+1}" ]; then
		echo "auto_prepend_file=${PHP_FPM_AUTO_PREPEND_FILE}"
	elif [ -n "${PHP_AUTO_PREPEND_FILE+1}" ]; then
		echo "auto_prepend_file=${PHP_AUTO_PREPEND_FILE}"
	fi
	if [ -n "${PHP_FPM_BCMATH_SCALE+1}" ]; then
		echo "bcmath.scale=${PHP_FPM_BCMATH_SCALE}"
	...
	if [ -n "${PHP_FPM_CLI_PAGER+1}" ]; then
		echo "cli.pager=${PHP_FPM_CLI_PAGER}"
	elif [ -n "${PHP_CLI_PAGER+1}" ]; then
		echo "cli.pager=${PHP_CLI_PAGER}"
	fi
	if [ -n "${PHP_FPM_CLI_PROMPT+1}" ]; then
		echo "cli.prompt=${PHP_FPM_CLI_PROMPT}"
	elif [ -n "${PHP_CLI_PROMPT+1}" ]; then
		echo "cli.prompt=${PHP_CLI_PROMPT}"
	fi
	if [ -n "${PHP_FPM_CURL_CAINFO+1}" ]; then
		echo "curl.cainfo=${PHP_FPM_CURL_CAINFO}"
	elif [ -n "${PHP_CURL_CAINFO+1}" ]; then
		echo "curl.cainfo=${PHP_CURL_CAINFO}"
	fi
	if [ -n "${PHP_FPM_DATE_DEFAULT_LATITUDE+1}" ]; then
		echo "date.default_latitude=${PHP_FPM_DATE_DEFAULT_LATITUDE}"
	elif [ -n "${PHP_DATE_DEFAULT_LATITUDE+1}" ]; then
		echo "date.default_latitude=${PHP_DATE_DEFAULT_LATITUDE}"
	fi
	if [ -n "${PHP_FPM_DATE_DEFAULT_LONGITUDE+1}" ]; then
		echo "date.default_longitude=${PHP_FPM_DATE_DEFAULT_LONGITUDE}"
	elif [ -n "${PHP_DATE_DEFAULT_LONGITUDE+1}" ]; then
		echo "date.default_longitude=${PHP_DATE_DEFAULT_LONGITUDE}"
	fi
	if [ -n "${PHP_FPM_DATE_SUNRISE_ZENITH+1}" ]; then
		echo "date.sunrise_zenith=${PHP_FPM_DATE_SUNRISE_ZENITH}"
	elif [ -n "${PHP_DATE_SUNRISE_ZENITH+1}" ]; then
		echo "date.sunrise_zenith=${PHP_DATE_SUNRISE_ZENITH}"
	fi
	if [ -n "${PHP_FPM_DATE_SUNSET_ZENITH+1}" ]; then
		echo "date.sunset_zenith=${PHP_FPM_DATE_SUNSET_ZENITH}"
	elif [ -n "${PHP_DATE_SUNSET_ZENITH+1}" ]; then
		echo "date.sunset_zenith=${PHP_DATE_SUNSET_ZENITH}"
	fi
	if [ -n "${PHP_FPM_DATE_TIMEZONE+1}" ]; then
		echo "date.timezone=${PHP_FPM_DATE_TIMEZONE}"
	elif [ -n "${PHP_DATE_TIMEZONE+1}" ]; then
		echo "date.timezone=${PHP_DATE_TIMEZONE}"
	fi
	if [ -n "${PHP_FPM_DBA_DEFAULT_HANDLER+1}" ]; then
		echo "dba.default_handler=${PHP_FPM_DBA_DEFAULT_HANDLER}"
	elif [ -n "${PHP_DBA_DEFAULT_HANDLER+1}" ]; then
		echo "dba.default_handler=${PHP_DBA_DEFAULT_HANDLER}"
	fi
	if [ -n "${PHP_FPM_DEFAULT_CHARSET+1}" ]; then
		echo "default_charset=${PHP_FPM_DEFAULT_CHARSET}"
	elif [ -n "${PHP_DEFAULT_CHARSET+1}" ]; then
		echo "default_charset=${PHP_DEFAULT_CHARSET}"
	fi
	if [ -n "${PHP_FPM_DEFAULT_MIMETYPE+1}" ]; then
		echo "default_mimetype=${PHP_FPM_DEFAULT_MIMETYPE}"
	elif [ -n "${PHP_DEFAULT_MIMETYPE+1}" ]; then
		echo "default_mimetype=${PHP_DEFAULT_MIMETYPE}"
	fi
	if [ -n "${PHP_FPM_DEFAULT_SOCKET_TIMEOUT+1}" ]; then
		echo "default_socket_timeout=${PHP_FPM_DEFAULT_SOCKET_TIMEOUT}"
	elif [ -n "${PHP_DEFAULT_SOCKET_TIMEOUT+1}" ]; then
		echo "default_socket_timeout=${PHP_DEFAULT_SOCKET_TIMEOUT}"
	fi
	if [ -n "${PHP_FPM_DISABLE_CLASSES+1}" ]; then
		echo "disable_classes=${PHP_FPM_DISABLE_CLASSES}"
	elif [ -n "${PHP_DISABLE_CLASSES+1}" ]; then
		echo "disable_classes=${PHP_DISABLE_CLASSES}"
	fi
	if [ -n "${PHP_FPM_DISABLE_FUNCTIONS+1}" ]; then
		echo "disable_functions=${PHP_FPM_DISABLE_FUNCTIONS}"
	elif [ -n "${PHP_DISABLE_FUNCTIONS+1}" ]; then
		echo "disable_functions=${PHP_DISABLE_FUNCTIONS}"
	fi
	if [ -n "${PHP_FPM_DISPLAY_ERRORS+1}" ]; then
		echo "display_errors=${PHP_FPM_DISPLAY_ERRORS}"
	elif [ -n "${PHP_DISPLAY_ERRORS+1}" ]; then
		echo "display_errors=${PHP_DISPLAY_ERRORS}"
	fi
	if [ -n "${PHP_FPM_DISPLAY_STARTUP_ERRORS+1}" ]; then
		echo "display_startup_errors=${PHP_FPM_DISPLAY_STARTUP_ERRORS}"
	elif [ -n "${PHP_DISPLAY_STARTUP_ERRORS+1}" ]; then
		echo "display_startup_errors=${PHP_DISPLAY_STARTUP_ERRORS}"
	fi
	if [ -n "${PHP_FPM_DOC_ROOT+1}" ]; then
		echo "doc_root=${PHP_FPM_DOC_ROOT}"
	elif [ -n "${PHP_DOC_ROOT+1}" ]; then
		echo "doc_root=${PHP_DOC_ROOT}"
	fi
	if [ -n "${PHP_FPM_DOCREF_EXT+1}" ]; then
		echo "docref_ext=${PHP_FPM_DOCREF_EXT}"
	elif [ -n "${PHP_DOCREF_EXT+1}" ]; then
		echo "docref_ext=${PHP_DOCREF_EXT}"
	fi
	if [ -n "${PHP_FPM_DOCREF_ROOT+1}" ]; then
		echo "docref_root=${PHP_FPM_DOCREF_ROOT}"
	elif [ -n "${PHP_DOCREF_ROOT+1}" ]; then
		echo "docref_root=${PHP_DOCREF_ROOT}"
	fi
	if [ -n "${PHP_FPM_ENABLE_DL+1}" ]; then
		echo "enable_dl=${PHP_FPM_ENABLE_DL}"
	elif [ -n "${PHP_ENABLE_DL+1}" ]; then
		echo "enable_dl=${PHP_ENABLE_DL}"
	fi
	if [ -n "${PHP_FPM_ENABLE_POST_DATA_READING+1}" ]; then
		echo "enable_post_data_reading=${PHP_FPM_ENABLE_POST_DATA_READING}"
	elif [ -n "${PHP_ENABLE_POST_DATA_READING+1}" ]; then
		echo "enable_post_data_reading=${PHP_ENABLE_POST_DATA_READING}"
	fi
	if [ -n "${PHP_FPM_ERROR_APPEND_STRING+1}" ]; then
		echo "error_append_string=${PHP_FPM_ERROR_APPEND_STRING}"
	...
	if [ -n "${PHP_FPM_ERROR_LOG_MODE+1}" ]; then
		echo "error_log_mode=${PHP_FPM_ERROR_LOG_MODE}"
	elif [ -n "${PHP_ERROR_LOG_MODE+1}" ]; then
		echo "error_log_mode=${PHP_ERROR_LOG_MODE}"
	fi
	if [ -n "${PHP_FPM_ERROR_PREPEND_STRING+1}" ]; then
		echo "error_prepend_string=${PHP_FPM_ERROR_PREPEND_STRING}"
	elif [ -n "${PHP_ERROR_PREPEND_STRING+1}" ]; then
		echo "error_prepend_string=${PHP_ERROR_PREPEND_STRING}"
	fi
	if [ -n "${PHP_FPM_ERROR_REPORTING+1}" ]; then
		echo "error_reporting=${PHP_FPM_ERROR_REPORTING}"
	...
	if [ -n "${PHP_FPM_EXIF_DECODE_JIS_MOTOROLA+1}" ]; then
		echo "exif.decode_jis_motorola=${PHP_FPM_EXIF_DECODE_JIS_MOTOROLA}"
	elif [ -n "${PHP_EXIF_DECODE_JIS_MOTOROLA+1}" ]; then
		echo "exif.decode_jis_motorola=${PHP_EXIF_DECODE_JIS_MOTOROLA}"
	fi
	if [ -n "${PHP_FPM_EXIF_DECODE_UNICODE_INTEL+1}" ]; then
		echo "exif.decode_unicode_intel=${PHP_FPM_EXIF_DECODE_UNICODE_INTEL}"
	elif [ -n "${PHP_EXIF_DECODE_UNICODE_INTEL+1}" ]; then
		echo "exif.decode_unicode_intel=${PHP_EXIF_DECODE_UNICODE_INTEL}"
	fi
	if [ -n "${PHP_FPM_EXIF_DECODE_UNICODE_MOTOROLA+1}" ]; then
		echo "exif.decode_unicode_motorola=${PHP_FPM_EXIF_DECODE_UNICODE_MOTOROLA}"
	elif [ -n "${PHP_EXIF_DECODE_UNICODE_MOTOROLA+1}" ]; then
		echo "exif.decode_unicode_motorola=${PHP_EXIF_DECODE_UNICODE_MOTOROLA}"
	fi
	if [ -n "${PHP_FPM_EXIF_ENCODE_JIS+1}" ]; then
		echo "exif.encode_jis=${PHP_FPM_EXIF_ENCODE_JIS}"
	elif [ -n "${PHP_EXIF_ENCODE_JIS+1}" ]; then
		echo "exif.encode_jis=${PHP_EXIF_ENCODE_JIS}"
	fi
	if [ -n "${PHP_FPM_EXIF_ENCODE_UNICODE+1}" ]; then
		echo "exif.encode_unicode=${PHP_FPM_EXIF_ENCODE_UNICODE}"
	...
	if [ -n "${PHP_FPM_EXTENSION_DIR+1}" ]; then
	...
	if [ -n "${PHP_FPM_FIBER_STACK_SIZE+1}" ]; then
		echo "fiber.stack_size=${PHP_FPM_FIBER_STACK_SIZE}"
	elif [ -n "${PHP_FIBER_STACK_SIZE+1}" ]; then
		echo "fiber.stack_size=${PHP_FIBER_STACK_SIZE}"
	fi
	...
	if [ -n "${PHP_FPM_FILTER_DEFAULT+1}" ]; then
		echo "filter.default=${PHP_FPM_FILTER_DEFAULT}"
	elif [ -n "${PHP_FILTER_DEFAULT+1}" ]; then
		echo "filter.default=${PHP_FILTER_DEFAULT}"
	fi
	if [ -n "${PHP_FPM_FILTER_DEFAULT_FLAGS+1}" ]; then
		echo "filter.default_flags=${PHP_FPM_FILTER_DEFAULT_FLAGS}"
	elif [ -n "${PHP_FILTER_DEFAULT_FLAGS+1}" ]; then
		echo "filter.default_flags=${PHP_FILTER_DEFAULT_FLAGS}"
	fi
	if [ -n "${PHP_FPM_FROM+1}" ]; then
		echo "from=${PHP_FPM_FROM}"
	elif [ -n "${PHP_FROM+1}" ]; then
		echo "from=${PHP_FROM}"
	fi
	if [ -n "${PHP_FPM_GD_JPEG_IGNORE_WARNING+1}" ]; then
		echo "gd.jpeg_ignore_warning=${PHP_FPM_GD_JPEG_IGNORE_WARNING}"
	...
	if [ -n "${PHP_FPM_GRPC_GRPC_TRACE+1}" ]; then
		echo "grpc.grpc_trace=${PHP_FPM_GRPC_GRPC_TRACE}"
	elif [ -n "${PHP_GRPC_GRPC_TRACE+1}" ]; then
		echo "grpc.grpc_trace=${PHP_GRPC_GRPC_TRACE}"
	fi
	if [ -n "${PHP_FPM_GRPC_GRPC_VERBOSITY+1}" ]; then
	...
	if [ -n "${PHP_FPM_HIGHLIGHT_DEFAULT+1}" ]; then
		echo "highlight.default=${PHP_FPM_HIGHLIGHT_DEFAULT}"
	elif [ -n "${PHP_HIGHLIGHT_DEFAULT+1}" ]; then
		echo "highlight.default=${PHP_HIGHLIGHT_DEFAULT}"
	fi
	if [ -n "${PHP_FPM_HIGHLIGHT_HTML+1}" ]; then
		echo "highlight.html=${PHP_FPM_HIGHLIGHT_HTML}"
	elif [ -n "${PHP_HIGHLIGHT_HTML+1}" ]; then
		echo "highlight.html=${PHP_HIGHLIGHT_HTML}"
	fi
	if [ -n "${PHP_FPM_HIGHLIGHT_KEYWORD+1}" ]; then
		echo "highlight.keyword=${PHP_FPM_HIGHLIGHT_KEYWORD}"
	elif [ -n "${PHP_HIGHLIGHT_KEYWORD+1}" ]; then
		echo "highlight.keyword=${PHP_HIGHLIGHT_KEYWORD}"
	fi
	if [ -n "${PHP_FPM_HIGHLIGHT_STRING+1}" ]; then
		echo "highlight.string=${PHP_FPM_HIGHLIGHT_STRING}"
	elif [ -n "${PHP_HIGHLIGHT_STRING+1}" ]; then
		echo "highlight.string=${PHP_HIGHLIGHT_STRING}"
	fi
	if [ -n "${PHP_FPM_HTML_ERRORS+1}" ]; then
		echo "html_errors=${PHP_FPM_HTML_ERRORS}"
	...
	if [ -n "${PHP_FPM_ICONV_INTERNAL_ENCODING+1}" ]; then
		echo "iconv.internal_encoding=${PHP_FPM_ICONV_INTERNAL_ENCODING}"
	elif [ -n "${PHP_ICONV_INTERNAL_ENCODING+1}" ]; then
		echo "iconv.internal_encoding=${PHP_ICONV_INTERNAL_ENCODING}"
	fi
	if [ -n "${PHP_FPM_ICONV_OUTPUT_ENCODING+1}" ]; then
		echo "iconv.output_encoding=${PHP_FPM_ICONV_OUTPUT_ENCODING}"
	elif [ -n "${PHP_ICONV_OUTPUT_ENCODING+1}" ]; then
		echo "iconv.output_encoding=${PHP_ICONV_OUTPUT_ENCODING}"
	fi
	if [ -n "${PHP_FPM_IGBINARY_COMPACT_STRINGS+1}" ]; then
		echo "igbinary.compact_strings=${PHP_FPM_IGBINARY_COMPACT_STRINGS}"
	elif [ -n "${PHP_IGBINARY_COMPACT_STRINGS+1}" ]; then
		echo "igbinary.compact_strings=${PHP_IGBINARY_COMPACT_STRINGS}"
	fi
	if [ -n "${PHP_FPM_IGNORE_REPEATED_ERRORS+1}" ]; then
		echo "ignore_repeated_errors=${PHP_FPM_IGNORE_REPEATED_ERRORS}"
	elif [ -n "${PHP_IGNORE_REPEATED_ERRORS+1}" ]; then
		echo "ignore_repeated_errors=${PHP_IGNORE_REPEATED_ERRORS}"
	fi
	if [ -n "${PHP_FPM_IGNORE_REPEATED_SOURCE+1}" ]; then
		echo "ignore_repeated_source=${PHP_FPM_IGNORE_REPEATED_SOURCE}"
	elif [ -n "${PHP_IGNORE_REPEATED_SOURCE+1}" ]; then
		echo "ignore_repeated_source=${PHP_IGNORE_REPEATED_SOURCE}"
	fi
	if [ -n "${PHP_FPM_IGNORE_USER_ABORT+1}" ]; then
	...
	if [ -n "${PHP_FPM_IMAGICK_PROGRESS_MONITOR+1}" ]; then
	...
	if [ -n "${PHP_FPM_IMAP_ENABLE_INSECURE_RSH+1}" ]; then
		echo "imap.enable_insecure_rsh=${PHP_FPM_IMAP_ENABLE_INSECURE_RSH}"
	elif [ -n "${PHP_IMAP_ENABLE_INSECURE_RSH+1}" ]; then
		echo "imap.enable_insecure_rsh=${PHP_IMAP_ENABLE_INSECURE_RSH}"
	fi
	if [ -n "${PHP_FPM_IMPLICIT_FLUSH+1}" ]; then
		echo "implicit_flush=${PHP_FPM_IMPLICIT_FLUSH}"
	elif [ -n "${PHP_IMPLICIT_FLUSH+1}" ]; then
		echo "implicit_flush=${PHP_IMPLICIT_FLUSH}"
	fi
	if [ -n "${PHP_FPM_INCLUDE_PATH+1}" ]; then
		echo "include_path=${PHP_FPM_INCLUDE_PATH}"
	elif [ -n "${PHP_INCLUDE_PATH+1}" ]; then
		echo "include_path=${PHP_INCLUDE_PATH}"
	fi
	if [ -n "${PHP_FPM_INPUT_ENCODING+1}" ]; then
		echo "input_encoding=${PHP_FPM_INPUT_ENCODING}"
	elif [ -n "${PHP_INPUT_ENCODING+1}" ]; then
		echo "input_encoding=${PHP_INPUT_ENCODING}"
	fi
	if [ -n "${PHP_FPM_INTERNAL_ENCODING+1}" ]; then
		echo "internal_encoding=${PHP_FPM_INTERNAL_ENCODING}"
	elif [ -n "${PHP_INTERNAL_ENCODING+1}" ]; then
		echo "internal_encoding=${PHP_INTERNAL_ENCODING}"
	fi
	if [ -n "${PHP_FPM_INTL_DEFAULT_LOCALE+1}" ]; then
		echo "intl.default_locale=${PHP_FPM_INTL_DEFAULT_LOCALE}"
	elif [ -n "${PHP_INTL_DEFAULT_LOCALE+1}" ]; then
		echo "intl.default_locale=${PHP_INTL_DEFAULT_LOCALE}"
	fi
	if [ -n "${PHP_FPM_INTL_ERROR_LEVEL+1}" ]; then
		echo "intl.error_level=${PHP_FPM_INTL_ERROR_LEVEL}"
	elif [ -n "${PHP_INTL_ERROR_LEVEL+1}" ]; then
		echo "intl.error_level=${PHP_INTL_ERROR_LEVEL}"
	fi
	if [ -n "${PHP_FPM_INTL_USE_EXCEPTIONS+1}" ]; then
		echo "intl.use_exceptions=${PHP_FPM_INTL_USE_EXCEPTIONS}"
	elif [ -n "${PHP_INTL_USE_EXCEPTIONS+1}" ]; then
		echo "intl.use_exceptions=${PHP_INTL_USE_EXCEPTIONS}"
	fi
	if [ -n "${PHP_FPM_LDAP_MAX_LINKS+1}" ]; then
	...
	if [ -n "${PHP_FPM_MAIL_ADD_X_HEADER+1}" ]; then
		echo "mail.add_x_header=${PHP_FPM_MAIL_ADD_X_HEADER}"
	elif [ -n "${PHP_MAIL_ADD_X_HEADER+1}" ]; then
		echo "mail.add_x_header=${PHP_MAIL_ADD_X_HEADER}"
	fi
	if [ -n "${PHP_FPM_MAIL_FORCE_EXTRA_PARAMETERS+1}" ]; then
		echo "mail.force_extra_parameters=${PHP_FPM_MAIL_FORCE_EXTRA_PARAMETERS}"
	elif [ -n "${PHP_MAIL_FORCE_EXTRA_PARAMETERS+1}" ]; then
		echo "mail.force_extra_parameters=${PHP_MAIL_FORCE_EXTRA_PARAMETERS}"
	fi
	if [ -n "${PHP_FPM_MAIL_LOG+1}" ]; then
		echo "mail.log=${PHP_FPM_MAIL_LOG}"
	elif [ -n "${PHP_MAIL_LOG+1}" ]; then
		echo "mail.log=${PHP_MAIL_LOG}"
	fi
	if [ -n "${PHP_FPM_MAIL_MIXED_LF_AND_CRLF+1}" ]; then
		echo "mail.mixed_lf_and_crlf=${PHP_FPM_MAIL_MIXED_LF_AND_CRLF}"
	elif [ -n "${PHP_MAIL_MIXED_LF_AND_CRLF+1}" ]; then
		echo "mail.mixed_lf_and_crlf=${PHP_MAIL_MIXED_LF_AND_CRLF}"
	fi
	if [ -n "${PHP_FPM_MAILPARSE_DEF_CHARSET+1}" ]; then
		echo "mailparse.def_charset=${PHP_FPM_MAILPARSE_DEF_CHARSET}"
	elif [ -n "${PHP_MAILPARSE_DEF_CHARSET+1}" ]; then
		echo "mailparse.def_charset=${PHP_MAILPARSE_DEF_CHARSET}"
	fi
	if [ -n "${PHP_FPM_MAX_EXECUTION_TIME+1}" ]; then
		echo "max_execution_time=${PHP_FPM_MAX_EXECUTION_TIME}"
	elif [ -n "${PHP_MAX_EXECUTION_TIME+1}" ]; then
		echo "max_execution_time=${PHP_MAX_EXECUTION_TIME}"
	fi
	if [ -n "${PHP_FPM_MAX_FILE_UPLOADS+1}" ]; then
		echo "max_file_uploads=${PHP_FPM_MAX_FILE_UPLOADS}"
	elif [ -n "${PHP_MAX_FILE_UPLOADS+1}" ]; then
		echo "max_file_uploads=${PHP_MAX_FILE_UPLOADS}"
	fi
	if [ -n "${PHP_FPM_MAX_INPUT_NESTING_LEVEL+1}" ]; then
		echo "max_input_nesting_level=${PHP_FPM_MAX_INPUT_NESTING_LEVEL}"
	elif [ -n "${PHP_MAX_INPUT_NESTING_LEVEL+1}" ]; then
		echo "max_input_nesting_level=${PHP_MAX_INPUT_NESTING_LEVEL}"
	fi
	if [ -n "${PHP_FPM_MAX_INPUT_TIME+1}" ]; then
		echo "max_input_time=${PHP_FPM_MAX_INPUT_TIME}"
	elif [ -n "${PHP_MAX_INPUT_TIME+1}" ]; then
		echo "max_input_time=${PHP_MAX_INPUT_TIME}"
	fi
	if [ -n "${PHP_FPM_MAX_INPUT_VARS+1}" ]; then
	...
	if [ -n "${PHP_FPM_MBSTRING_ENCODING_TRANSLATION+1}" ]; then
		echo "mbstring.encoding_translation=${PHP_FPM_MBSTRING_ENCODING_TRANSLATION}"
	...
	if [ -n "${PHP_FPM_MBSTRING_HTTP_OUTPUT+1}" ]; then
		echo "mbstring.http_output=${PHP_FPM_MBSTRING_HTTP_OUTPUT}"
	elif [ -n "${PHP_MBSTRING_HTTP_OUTPUT+1}" ]; then
		echo "mbstring.http_output=${PHP_MBSTRING_HTTP_OUTPUT}"
	fi
	if [ -n "${PHP_FPM_MBSTRING_HTTP_OUTPUT_CONV_MIMETYPES+1}" ]; then
		echo "mbstring.http_output_conv_mimetypes=${PHP_FPM_MBSTRING_HTTP_OUTPUT_CONV_MIMETYPES}"
	...
	if [ -n "${PHP_FPM_MBSTRING_INTERNAL_ENCODING+1}" ]; then
		echo "mbstring.internal_encoding=${PHP_FPM_MBSTRING_INTERNAL_ENCODING}"
	elif [ -n "${PHP_MBSTRING_INTERNAL_ENCODING+1}" ]; then
		echo "mbstring.internal_encoding=${PHP_MBSTRING_INTERNAL_ENCODING}"
	fi
	if [ -n "${PHP_FPM_MBSTRING_LANGUAGE+1}" ]; then
	...
	if [ -n "${PHP_FPM_MBSTRING_SUBSTITUTE_CHARACTER+1}" ]; then
		echo "mbstring.substitute_character=${PHP_FPM_MBSTRING_SUBSTITUTE_CHARACTER}"
	elif [ -n "${PHP_MBSTRING_SUBSTITUTE_CHARACTER+1}" ]; then
		echo "mbstring.substitute_character=${PHP_MBSTRING_SUBSTITUTE_CHARACTER}"
	fi
	if [ -n "${PHP_FPM_MCRYPT_ALGORITHMS_DIR+1}" ]; then
		echo "mcrypt.algorithms_dir=${PHP_FPM_MCRYPT_ALGORITHMS_DIR}"
	elif [ -n "${PHP_MCRYPT_ALGORITHMS_DIR+1}" ]; then
		echo "mcrypt.algorithms_dir=${PHP_MCRYPT_ALGORITHMS_DIR}"
	fi
	if [ -n "${PHP_FPM_MCRYPT_MODES_DIR+1}" ]; then
		echo "mcrypt.modes_dir=${PHP_FPM_MCRYPT_MODES_DIR}"
	elif [ -n "${PHP_MCRYPT_MODES_DIR+1}" ]; then
		echo "mcrypt.modes_dir=${PHP_MCRYPT_MODES_DIR}"
	fi
	if [ -n "${PHP_FPM_MEMCACHE_ALLOW_FAILOVER+1}" ]; then
		echo "memcache.allow_failover=${PHP_FPM_MEMCACHE_ALLOW_FAILOVER}"
	elif [ -n "${PHP_MEMCACHE_ALLOW_FAILOVER+1}" ]; then
		echo "memcache.allow_failover=${PHP_MEMCACHE_ALLOW_FAILOVER}"
	fi
	if [ -n "${PHP_FPM_MEMCACHE_CHUNK_SIZE+1}" ]; then
		echo "memcache.chunk_size=${PHP_FPM_MEMCACHE_CHUNK_SIZE}"
	elif [ -n "${PHP_MEMCACHE_CHUNK_SIZE+1}" ]; then
		echo "memcache.chunk_size=${PHP_MEMCACHE_CHUNK_SIZE}"
	fi
	if [ -n "${PHP_FPM_MEMCACHE_COMPRESS_THRESHOLD+1}" ]; then
		echo "memcache.compress_threshold=${PHP_FPM_MEMCACHE_COMPRESS_THRESHOLD}"
	elif [ -n "${PHP_MEMCACHE_COMPRESS_THRESHOLD+1}" ]; then
		echo "memcache.compress_threshold=${PHP_MEMCACHE_COMPRESS_THRESHOLD}"
	fi
	if [ -n "${PHP_FPM_MEMCACHE_DEFAULT_PORT+1}" ]; then
		echo "memcache.default_port=${PHP_FPM_MEMCACHE_DEFAULT_PORT}"
	elif [ -n "${PHP_MEMCACHE_DEFAULT_PORT+1}" ]; then
		echo "memcache.default_port=${PHP_MEMCACHE_DEFAULT_PORT}"
	fi
	if [ -n "${PHP_FPM_MEMCACHE_HASH_FUNCTION+1}" ]; then
		echo "memcache.hash_function=${PHP_FPM_MEMCACHE_HASH_FUNCTION}"
	elif [ -n "${PHP_MEMCACHE_HASH_FUNCTION+1}" ]; then
		echo "memcache.hash_function=${PHP_MEMCACHE_HASH_FUNCTION}"
	fi
	if [ -n "${PHP_FPM_MEMCACHE_HASH_STRATEGY+1}" ]; then
		echo "memcache.hash_strategy=${PHP_FPM_MEMCACHE_HASH_STRATEGY}"
	elif [ -n "${PHP_MEMCACHE_HASH_STRATEGY+1}" ]; then
		echo "memcache.hash_strategy=${PHP_MEMCACHE_HASH_STRATEGY}"
	fi
	if [ -n "${PHP_FPM_MEMCACHE_LOCK_TIMEOUT+1}" ]; then
		echo "memcache.lock_timeout=${PHP_FPM_MEMCACHE_LOCK_TIMEOUT}"
	elif [ -n "${PHP_MEMCACHE_LOCK_TIMEOUT+1}" ]; then
		echo "memcache.lock_timeout=${PHP_MEMCACHE_LOCK_TIMEOUT}"
	fi
	if [ -n "${PHP_FPM_MEMCACHE_MAX_FAILOVER_ATTEMPTS+1}" ]; then
		echo "memcache.max_failover_attempts=${PHP_FPM_MEMCACHE_MAX_FAILOVER_ATTEMPTS}"
	...
	if [ -n "${PHP_FPM_MEMCACHE_REDUNDANCY+1}" ]; then
		echo "memcache.redundancy=${PHP_FPM_MEMCACHE_REDUNDANCY}"
	...
	if [ -n "${PHP_FPM_MEMCACHED_COMPRESSION_TYPE+1}" ]; then
		echo "memcached.compression_type=${PHP_FPM_MEMCACHED_COMPRESSION_TYPE}"
	...
	if [ -n "${PHP_FPM_MEMCACHED_SESS_CONSISTENT_HASH+1}" ]; then
		echo "memcached.sess_consistent_hash=${PHP_FPM_MEMCACHED_SESS_CONSISTENT_HASH}"
	...
	if [ -n "${PHP_FPM_MEMCACHED_SESS_LOCK_MAX_WAIT+1}" ]; then
		echo "memcached.sess_lock_max_wait=${PHP_FPM_MEMCACHED_SESS_LOCK_MAX_WAIT}"
	...
	if [ -n "${PHP_FPM_MEMCACHED_SESS_NUMBER_OF_REPLICAS+1}" ]; then
		echo "memcached.sess_number_of_replicas=${PHP_FPM_MEMCACHED_SESS_NUMBER_OF_REPLICAS}"
	...
	if [ -n "${PHP_FPM_MEMCACHED_SESS_RANDOMIZE_REPLICA_READ+1}" ]; then
		echo "memcached.sess_randomize_replica_read=${PHP_FPM_MEMCACHED_SESS_RANDOMIZE_REPLICA_READ}"
	...
	if [ -n "${PHP_FPM_MEMCACHED_SESS_SASL_USERNAME+1}" ]; then
		echo "memcached.sess_sasl_username=${PHP_FPM_MEMCACHED_SESS_SASL_USERNAME}"
	...
	if [ -n "${PHP_FPM_MSGPACK_ILLEGAL_KEY_INSERT+1}" ]; then
		echo "msgpack.illegal_key_insert=${PHP_FPM_MSGPACK_ILLEGAL_KEY_INSERT}"
	elif [ -n "${PHP_MSGPACK_ILLEGAL_KEY_INSERT+1}" ]; then
		echo "msgpack.illegal_key_insert=${PHP_MSGPACK_ILLEGAL_KEY_INSERT}"
	fi
	if [ -n "${PHP_FPM_MSGPACK_PHP_ONLY+1}" ]; then
		echo "msgpack.php_only=${PHP_FPM_MSGPACK_PHP_ONLY}"
	elif [ -n "${PHP_MSGPACK_PHP_ONLY+1}" ]; then
		echo "msgpack.php_only=${PHP_MSGPACK_PHP_ONLY}"
	fi
	if [ -n "${PHP_FPM_MSGPACK_USE_STR8_SERIALIZATION+1}" ]; then
	...
	if [ -n "${PHP_FPM_MYSQLI_ALLOW_PERSISTENT+1}" ]; then
		echo "mysqli.allow_persistent=${PHP_FPM_MYSQLI_ALLOW_PERSISTENT}"
	elif [ -n "${PHP_MYSQLI_ALLOW_PERSISTENT+1}" ]; then
		echo "mysqli.allow_persistent=${PHP_MYSQLI_ALLOW_PERSISTENT}"
	fi
	if [ -n "${PHP_FPM_MYSQLI_DEFAULT_HOST+1}" ]; then
		echo "mysqli.default_host=${PHP_FPM_MYSQLI_DEFAULT_HOST}"
	elif [ -n "${PHP_MYSQLI_DEFAULT_HOST+1}" ]; then
		echo "mysqli.default_host=${PHP_MYSQLI_DEFAULT_HOST}"
	fi
	if [ -n "${PHP_FPM_MYSQLI_DEFAULT_PORT+1}" ]; then
		echo "mysqli.default_port=${PHP_FPM_MYSQLI_DEFAULT_PORT}"
	elif [ -n "${PHP_MYSQLI_DEFAULT_PORT+1}" ]; then
		echo "mysqli.default_port=${PHP_MYSQLI_DEFAULT_PORT}"
	fi
	if [ -n "${PHP_FPM_MYSQLI_DEFAULT_PW+1}" ]; then
		echo "mysqli.default_pw=${PHP_FPM_MYSQLI_DEFAULT_PW}"
	elif [ -n "${PHP_MYSQLI_DEFAULT_PW+1}" ]; then
		echo "mysqli.default_pw=${PHP_MYSQLI_DEFAULT_PW}"
	fi
	if [ -n "${PHP_FPM_MYSQLI_DEFAULT_SOCKET+1}" ]; then
		echo "mysqli.default_socket=${PHP_FPM_MYSQLI_DEFAULT_SOCKET}"
	elif [ -n "${PHP_MYSQLI_DEFAULT_SOCKET+1}" ]; then
		echo "mysqli.default_socket=${PHP_MYSQLI_DEFAULT_SOCKET}"
	fi
	if [ -n "${PHP_FPM_MYSQLI_DEFAULT_USER+1}" ]; then
		echo "mysqli.default_user=${PHP_FPM_MYSQLI_DEFAULT_USER}"
	elif [ -n "${PHP_MYSQLI_DEFAULT_USER+1}" ]; then
		echo "mysqli.default_user=${PHP_MYSQLI_DEFAULT_USER}"
	fi
	if [ -n "${PHP_FPM_MYSQLI_LOCAL_INFILE_DIRECTORY+1}" ]; then
		echo "mysqli.local_infile_directory=${PHP_FPM_MYSQLI_LOCAL_INFILE_DIRECTORY}"
	elif [ -n "${PHP_MYSQLI_LOCAL_INFILE_DIRECTORY+1}" ]; then
		echo "mysqli.local_infile_directory=${PHP_MYSQLI_LOCAL_INFILE_DIRECTORY}"
	fi
	if [ -n "${PHP_FPM_MYSQLI_MAX_LINKS+1}" ]; then
		echo "mysqli.max_links=${PHP_FPM_MYSQLI_MAX_LINKS}"
	elif [ -n "${PHP_MYSQLI_MAX_LINKS+1}" ]; then
		echo "mysqli.max_links=${PHP_MYSQLI_MAX_LINKS}"
	fi
	if [ -n "${PHP_FPM_MYSQLI_MAX_PERSISTENT+1}" ]; then
		echo "mysqli.max_persistent=${PHP_FPM_MYSQLI_MAX_PERSISTENT}"
	elif [ -n "${PHP_MYSQLI_MAX_PERSISTENT+1}" ]; then
		echo "mysqli.max_persistent=${PHP_MYSQLI_MAX_PERSISTENT}"
	fi
	if [ -n "${PHP_FPM_MYSQLI_ROLLBACK_ON_CACHED_PLINK+1}" ]; then
		echo "mysqli.rollback_on_cached_plink=${PHP_FPM_MYSQLI_ROLLBACK_ON_CACHED_PLINK}"
	...
	if [ -n "${PHP_FPM_MYSQLND_COLLECT_MEMORY_STATISTICS+1}" ]; then
		echo "mysqlnd.collect_memory_statistics=${PHP_FPM_MYSQLND_COLLECT_MEMORY_STATISTICS}"
	...
	if [ -n "${PHP_FPM_MYSQLND_COLLECT_STATISTICS+1}" ]; then
		echo "mysqlnd.collect_statistics=${PHP_FPM_MYSQLND_COLLECT_STATISTICS}"
	elif [ -n "${PHP_MYSQLND_COLLECT_STATISTICS+1}" ]; then
		echo "mysqlnd.collect_statistics=${PHP_MYSQLND_COLLECT_STATISTICS}"
	fi
	if [ -n "${PHP_FPM_MYSQLND_DEBUG+1}" ]; then
		echo "mysqlnd.debug=${PHP_FPM_MYSQLND_DEBUG}"
	elif [ -n "${PHP_MYSQLND_DEBUG+1}" ]; then
		echo "mysqlnd.debug=${PHP_MYSQLND_DEBUG}"
	fi
	if [ -n "${PHP_FPM_MYSQLND_LOG_MASK+1}" ]; then
		echo "mysqlnd.log_mask=${PHP_FPM_MYSQLND_LOG_MASK}"
	elif [ -n "${PHP_MYSQLND_LOG_MASK+1}" ]; then
		echo "mysqlnd.log_mask=${PHP_MYSQLND_LOG_MASK}"
	fi
	if [ -n "${PHP_FPM_MYSQLND_MEMPOOL_DEFAULT_SIZE+1}" ]; then
		echo "mysqlnd.mempool_default_size=${PHP_FPM_MYSQLND_MEMPOOL_DEFAULT_SIZE}"
	elif [ -n "${PHP_MYSQLND_MEMPOOL_DEFAULT_SIZE+1}" ]; then
		echo "mysqlnd.mempool_default_size=${PHP_MYSQLND_MEMPOOL_DEFAULT_SIZE}"
	fi
	if [ -n "${PHP_FPM_MYSQLND_NET_CMD_BUFFER_SIZE+1}" ]; then
		echo "mysqlnd.net_cmd_buffer_size=${PHP_FPM_MYSQLND_NET_CMD_BUFFER_SIZE}"
	elif [ -n "${PHP_MYSQLND_NET_CMD_BUFFER_SIZE+1}" ]; then
		echo "mysqlnd.net_cmd_buffer_size=${PHP_MYSQLND_NET_CMD_BUFFER_SIZE}"
	fi
	if [ -n "${PHP_FPM_MYSQLND_NET_READ_BUFFER_SIZE+1}" ]; then
		echo "mysqlnd.net_read_buffer_size=${PHP_FPM_MYSQLND_NET_READ_BUFFER_SIZE}"
	elif [ -n "${PHP_MYSQLND_NET_READ_BUFFER_SIZE+1}" ]; then
		echo "mysqlnd.net_read_buffer_size=${PHP_MYSQLND_NET_READ_BUFFER_SIZE}"
	fi
	if [ -n "${PHP_FPM_MYSQLND_NET_READ_TIMEOUT+1}" ]; then
		echo "mysqlnd.net_read_timeout=${PHP_FPM_MYSQLND_NET_READ_TIMEOUT}"
	elif [ -n "${PHP_MYSQLND_NET_READ_TIMEOUT+1}" ]; then
		echo "mysqlnd.net_read_timeout=${PHP_MYSQLND_NET_READ_TIMEOUT}"
	fi
	if [ -n "${PHP_FPM_MYSQLND_SHA256_SERVER_PUBLIC_KEY+1}" ]; then
		echo "mysqlnd.sha256_server_public_key=${PHP_FPM_MYSQLND_SHA256_SERVER_PUBLIC_KEY}"
	...
	if [ -n "${PHP_FPM_MYSQLND_TRACE_ALLOC+1}" ]; then
	...
	if [ -n "${PHP_FPM_ODBC_CHECK_PERSISTENT+1}" ]; then
		echo "odbc.check_persistent=${PHP_FPM_ODBC_CHECK_PERSISTENT}"
	elif [ -n "${PHP_ODBC_CHECK_PERSISTENT+1}" ]; then
		echo "odbc.check_persistent=${PHP_ODBC_CHECK_PERSISTENT}"
	fi
	if [ -n "${PHP_FPM_ODBC_DEFAULT_CURSORTYPE+1}" ]; then
		echo "odbc.default_cursortype=${PHP_FPM_ODBC_DEFAULT_CURSORTYPE}"
	elif [ -n "${PHP_ODBC_DEFAULT_CURSORTYPE+1}" ]; then
		echo "odbc.default_cursortype=${PHP_ODBC_DEFAULT_CURSORTYPE}"
	fi
	if [ -n "${PHP_FPM_ODBC_DEFAULT_DB+1}" ]; then
		echo "odbc.default_db=${PHP_FPM_ODBC_DEFAULT_DB}"
	elif [ -n "${PHP_ODBC_DEFAULT_DB+1}" ]; then
		echo "odbc.default_db=${PHP_ODBC_DEFAULT_DB}"
	fi
	if [ -n "${PHP_FPM_ODBC_DEFAULT_PW+1}" ]; then
		echo "odbc.default_pw=${PHP_FPM_ODBC_DEFAULT_PW}"
	elif [ -n "${PHP_ODBC_DEFAULT_PW+1}" ]; then
		echo "odbc.default_pw=${PHP_ODBC_DEFAULT_PW}"
	fi
	if [ -n "${PHP_FPM_ODBC_DEFAULT_USER+1}" ]; then
		echo "odbc.default_user=${PHP_FPM_ODBC_DEFAULT_USER}"
	elif [ -n "${PHP_ODBC_DEFAULT_USER+1}" ]; then
		echo "odbc.default_user=${PHP_ODBC_DEFAULT_USER}"
	fi
	if [ -n "${PHP_FPM_ODBC_DEFAULTBINMODE+1}" ]; then
		echo "odbc.defaultbinmode=${PHP_FPM_ODBC_DEFAULTBINMODE}"
	elif [ -n "${PHP_ODBC_DEFAULTBINMODE+1}" ]; then
		echo "odbc.defaultbinmode=${PHP_ODBC_DEFAULTBINMODE}"
	fi
	if [ -n "${PHP_FPM_ODBC_DEFAULTLRL+1}" ]; then
		echo "odbc.defaultlrl=${PHP_FPM_ODBC_DEFAULTLRL}"
	elif [ -n "${PHP_ODBC_DEFAULTLRL+1}" ]; then
		echo "odbc.defaultlrl=${PHP_ODBC_DEFAULTLRL}"
	fi
	if [ -n "${PHP_FPM_ODBC_MAX_LINKS+1}" ]; then
		echo "odbc.max_links=${PHP_FPM_ODBC_MAX_LINKS}"
	elif [ -n "${PHP_ODBC_MAX_LINKS+1}" ]; then
		echo "odbc.max_links=${PHP_ODBC_MAX_LINKS}"
	fi
	if [ -n "${PHP_FPM_ODBC_MAX_PERSISTENT+1}" ]; then
		echo "odbc.max_persistent=${PHP_FPM_ODBC_MAX_PERSISTENT}"
	elif [ -n "${PHP_ODBC_MAX_PERSISTENT+1}" ]; then
		echo "odbc.max_persistent=${PHP_ODBC_MAX_PERSISTENT}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_BLACKLIST_FILENAME+1}" ]; then
		echo "opcache.blacklist_filename=${PHP_FPM_OPCACHE_BLACKLIST_FILENAME}"
	elif [ -n "${PHP_OPCACHE_BLACKLIST_FILENAME+1}" ]; then
		echo "opcache.blacklist_filename=${PHP_OPCACHE_BLACKLIST_FILENAME}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_DUPS_FIX+1}" ]; then
		echo "opcache.dups_fix=${PHP_FPM_OPCACHE_DUPS_FIX}"
	elif [ -n "${PHP_OPCACHE_DUPS_FIX+1}" ]; then
		echo "opcache.dups_fix=${PHP_OPCACHE_DUPS_FIX}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_ENABLE+1}" ]; then
		echo "opcache.enable=${PHP_FPM_OPCACHE_ENABLE}"
	elif [ -n "${PHP_OPCACHE_ENABLE+1}" ]; then
		echo "opcache.enable=${PHP_OPCACHE_ENABLE}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_ENABLE_CLI+1}" ]; then
		echo "opcache.enable_cli=${PHP_FPM_OPCACHE_ENABLE_CLI}"
	elif [ -n "${PHP_OPCACHE_ENABLE_CLI+1}" ]; then
		echo "opcache.enable_cli=${PHP_OPCACHE_ENABLE_CLI}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_ENABLE_FILE_OVERRIDE+1}" ]; then
		echo "opcache.enable_file_override=${PHP_FPM_OPCACHE_ENABLE_FILE_OVERRIDE}"
	elif [ -n "${PHP_OPCACHE_ENABLE_FILE_OVERRIDE+1}" ]; then
		echo "opcache.enable_file_override=${PHP_OPCACHE_ENABLE_FILE_OVERRIDE}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_ERROR_LOG+1}" ]; then
		echo "opcache.error_log=${PHP_FPM_OPCACHE_ERROR_LOG}"
	...
	if [ -n "${PHP_FPM_OPCACHE_FORCE_RESTART_TIMEOUT+1}" ]; then
		echo "opcache.force_restart_timeout=${PHP_FPM_OPCACHE_FORCE_RESTART_TIMEOUT}"
	...
	if [ -n "${PHP_FPM_OPCACHE_JIT_MAX_TRACE_LENGTH+1}" ]; then
		echo "opcache.jit_max_trace_length=${PHP_FPM_OPCACHE_JIT_MAX_TRACE_LENGTH}"
	elif [ -n "${PHP_OPCACHE_JIT_MAX_TRACE_LENGTH+1}" ]; then
		echo "opcache.jit_max_trace_length=${PHP_OPCACHE_JIT_MAX_TRACE_LENGTH}"
	fi
	...
	if [ -n "${PHP_FPM_OPCACHE_LOG_VERBOSITY_LEVEL+1}" ]; then
		echo "opcache.log_verbosity_level=${PHP_FPM_OPCACHE_LOG_VERBOSITY_LEVEL}"
	elif [ -n "${PHP_OPCACHE_LOG_VERBOSITY_LEVEL+1}" ]; then
		echo "opcache.log_verbosity_level=${PHP_OPCACHE_LOG_VERBOSITY_LEVEL}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_MAX_ACCELERATED_FILES+1}" ]; then
		echo "opcache.max_accelerated_files=${PHP_FPM_OPCACHE_MAX_ACCELERATED_FILES}"
	elif [ -n "${PHP_OPCACHE_MAX_ACCELERATED_FILES+1}" ]; then
		echo "opcache.max_accelerated_files=${PHP_OPCACHE_MAX_ACCELERATED_FILES}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_MAX_FILE_SIZE+1}" ]; then
		echo "opcache.max_file_size=${PHP_FPM_OPCACHE_MAX_FILE_SIZE}"
	elif [ -n "${PHP_OPCACHE_MAX_FILE_SIZE+1}" ]; then
		echo "opcache.max_file_size=${PHP_OPCACHE_MAX_FILE_SIZE}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_MAX_WASTED_PERCENTAGE+1}" ]; then
		echo "opcache.max_wasted_percentage=${PHP_FPM_OPCACHE_MAX_WASTED_PERCENTAGE}"
	elif [ -n "${PHP_OPCACHE_MAX_WASTED_PERCENTAGE+1}" ]; then
		echo "opcache.max_wasted_percentage=${PHP_OPCACHE_MAX_WASTED_PERCENTAGE}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_MEMORY_CONSUMPTION+1}" ]; then
		echo "opcache.memory_consumption=${PHP_FPM_OPCACHE_MEMORY_CONSUMPTION}"
	...
	if [ -n "${PHP_FPM_OPCACHE_PREFERRED_MEMORY_MODEL+1}" ]; then
		echo "opcache.preferred_memory_model=${PHP_FPM_OPCACHE_PREFERRED_MEMORY_MODEL}"
	...
	if [ -n "${PHP_FPM_OPCACHE_REVALIDATE_FREQ+1}" ]; then
		echo "opcache.revalidate_freq=${PHP_FPM_OPCACHE_REVALIDATE_FREQ}"
	elif [ -n "${PHP_OPCACHE_REVALIDATE_FREQ+1}" ]; then
		echo "opcache.revalidate_freq=${PHP_OPCACHE_REVALIDATE_FREQ}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_REVALIDATE_PATH+1}" ]; then
		echo "opcache.revalidate_path=${PHP_FPM_OPCACHE_REVALIDATE_PATH}"
	elif [ -n "${PHP_OPCACHE_REVALIDATE_PATH+1}" ]; then
		echo "opcache.revalidate_path=${PHP_OPCACHE_REVALIDATE_PATH}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_SAVE_COMMENTS+1}" ]; then
		echo "opcache.save_comments=${PHP_FPM_OPCACHE_SAVE_COMMENTS}"
	elif [ -n "${PHP_OPCACHE_SAVE_COMMENTS+1}" ]; then
		echo "opcache.save_comments=${PHP_OPCACHE_SAVE_COMMENTS}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_USE_CWD+1}" ]; then
		echo "opcache.use_cwd=${PHP_FPM_OPCACHE_USE_CWD}"
	elif [ -n "${PHP_OPCACHE_USE_CWD+1}" ]; then
		echo "opcache.use_cwd=${PHP_OPCACHE_USE_CWD}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_VALIDATE_PERMISSION+1}" ]; then
		echo "opcache.validate_permission=${PHP_FPM_OPCACHE_VALIDATE_PERMISSION}"
	elif [ -n "${PHP_OPCACHE_VALIDATE_PERMISSION+1}" ]; then
		echo "opcache.validate_permission=${PHP_OPCACHE_VALIDATE_PERMISSION}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_VALIDATE_ROOT+1}" ]; then
		echo "opcache.validate_root=${PHP_FPM_OPCACHE_VALIDATE_ROOT}"
	elif [ -n "${PHP_OPCACHE_VALIDATE_ROOT+1}" ]; then
		echo "opcache.validate_root=${PHP_OPCACHE_VALIDATE_ROOT}"
	fi
	if [ -n "${PHP_FPM_OPCACHE_VALIDATE_TIMESTAMPS+1}" ]; then
		echo "opcache.validate_timestamps=${PHP_FPM_OPCACHE_VALIDATE_TIMESTAMPS}"
	elif [ -n "${PHP_OPCACHE_VALIDATE_TIMESTAMPS+1}" ]; then
		echo "opcache.validate_timestamps=${PHP_OPCACHE_VALIDATE_TIMESTAMPS}"
	fi
	if [ -n "${PHP_FPM_OPEN_BASEDIR+1}" ]; then
		echo "open_basedir=${PHP_FPM_OPEN_BASEDIR}"
	elif [ -n "${PHP_OPEN_BASEDIR+1}" ]; then
		echo "open_basedir=${PHP_OPEN_BASEDIR}"
	fi
	if [ -n "${PHP_FPM_OPENSSL_CAFILE+1}" ]; then
		echo "openssl.cafile=${PHP_FPM_OPENSSL_CAFILE}"
	elif [ -n "${PHP_OPENSSL_CAFILE+1}" ]; then
		echo "openssl.cafile=${PHP_OPENSSL_CAFILE}"
	fi
	if [ -n "${PHP_FPM_OPENSSL_CAPATH+1}" ]; then
		echo "openssl.capath=${PHP_FPM_OPENSSL_CAPATH}"
	elif [ -n "${PHP_OPENSSL_CAPATH+1}" ]; then
		echo "openssl.capath=${PHP_OPENSSL_CAPATH}"
	fi
	if [ -n "${PHP_FPM_OPENTELEMETRY_ALLOW_STACK_EXTENSION+1}" ]; then
		echo "opentelemetry.allow_stack_extension=${PHP_FPM_OPENTELEMETRY_ALLOW_STACK_EXTENSION}"
	...
	if [ -n "${PHP_FPM_OPENTELEMETRY_ATTR_HOOKS_ENABLED+1}" ]; then
		echo "opentelemetry.attr_hooks_enabled=${PHP_FPM_OPENTELEMETRY_ATTR_HOOKS_ENABLED}"
	...
	if [ -n "${PHP_FPM_OPENTELEMETRY_ATTR_POST_HANDLER_FUNCTION+1}" ]; then
		echo "opentelemetry.attr_post_handler_function=${PHP_FPM_OPENTELEMETRY_ATTR_POST_HANDLER_FUNCTION}"
	...
	if [ -n "${PHP_FPM_OPENTELEMETRY_ATTR_PRE_HANDLER_FUNCTION+1}" ]; then
		echo "opentelemetry.attr_pre_handler_function=${PHP_FPM_OPENTELEMETRY_ATTR_PRE_HANDLER_FUNCTION}"
	...
	if [ -n "${PHP_FPM_OPENTELEMETRY_CONFLICTS+1}" ]; then
		echo "opentelemetry.conflicts=${PHP_FPM_OPENTELEMETRY_CONFLICTS}"
	elif [ -n "${PHP_OPENTELEMETRY_CONFLICTS+1}" ]; then
		echo "opentelemetry.conflicts=${PHP_OPENTELEMETRY_CONFLICTS}"
	fi
	if [ -n "${PHP_FPM_OPENTELEMETRY_DISPLAY_WARNINGS+1}" ]; then
		echo "opentelemetry.display_warnings=${PHP_FPM_OPENTELEMETRY_DISPLAY_WARNINGS}"
	elif [ -n "${PHP_OPENTELEMETRY_DISPLAY_WARNINGS+1}" ]; then
		echo "opentelemetry.display_warnings=${PHP_OPENTELEMETRY_DISPLAY_WARNINGS}"
	fi
	if [ -n "${PHP_FPM_OPENTELEMETRY_VALIDATE_HOOK_FUNCTIONS+1}" ]; then
		echo "opentelemetry.validate_hook_functions=${PHP_FPM_OPENTELEMETRY_VALIDATE_HOOK_FUNCTIONS}"
	...
	if [ -n "${PHP_FPM_OUTPUT_BUFFERING+1}" ]; then
		echo "output_buffering=${PHP_FPM_OUTPUT_BUFFERING}"
	elif [ -n "${PHP_OUTPUT_BUFFERING+1}" ]; then
		echo "output_buffering=${PHP_OUTPUT_BUFFERING}"
	fi
	if [ -n "${PHP_FPM_OUTPUT_ENCODING+1}" ]; then
		echo "output_encoding=${PHP_FPM_OUTPUT_ENCODING}"
	elif [ -n "${PHP_OUTPUT_ENCODING+1}" ]; then
		echo "output_encoding=${PHP_OUTPUT_ENCODING}"
	fi
	if [ -n "${PHP_FPM_OUTPUT_HANDLER+1}" ]; then
		echo "output_handler=${PHP_FPM_OUTPUT_HANDLER}"
	...
	if [ -n "${PHP_FPM_PDO_MYSQL_DEFAULT_SOCKET+1}" ]; then
		echo "pdo_mysql.default_socket=${PHP_FPM_PDO_MYSQL_DEFAULT_SOCKET}"
	elif [ -n "${PHP_PDO_MYSQL_DEFAULT_SOCKET+1}" ]; then
		echo "pdo_mysql.default_socket=${PHP_PDO_MYSQL_DEFAULT_SOCKET}"
	fi
	if [ -n "${PHP_FPM_PGSQL_ALLOW_PERSISTENT+1}" ]; then
		echo "pgsql.allow_persistent=${PHP_FPM_PGSQL_ALLOW_PERSISTENT}"
	elif [ -n "${PHP_PGSQL_ALLOW_PERSISTENT+1}" ]; then
		echo "pgsql.allow_persistent=${PHP_PGSQL_ALLOW_PERSISTENT}"
	fi
	if [ -n "${PHP_FPM_PGSQL_AUTO_RESET_PERSISTENT+1}" ]; then
		echo "pgsql.auto_reset_persistent=${PHP_FPM_PGSQL_AUTO_RESET_PERSISTENT}"
	elif [ -n "${PHP_PGSQL_AUTO_RESET_PERSISTENT+1}" ]; then
		echo "pgsql.auto_reset_persistent=${PHP_PGSQL_AUTO_RESET_PERSISTENT}"
	fi
	if [ -n "${PHP_FPM_PGSQL_IGNORE_NOTICE+1}" ]; then
		echo "pgsql.ignore_notice=${PHP_FPM_PGSQL_IGNORE_NOTICE}"
	elif [ -n "${PHP_PGSQL_IGNORE_NOTICE+1}" ]; then
		echo "pgsql.ignore_notice=${PHP_PGSQL_IGNORE_NOTICE}"
	fi
	if [ -n "${PHP_FPM_PGSQL_LOG_NOTICE+1}" ]; then
		echo "pgsql.log_notice=${PHP_FPM_PGSQL_LOG_NOTICE}"
	elif [ -n "${PHP_PGSQL_LOG_NOTICE+1}" ]; then
		echo "pgsql.log_notice=${PHP_PGSQL_LOG_NOTICE}"
	fi
	if [ -n "${PHP_FPM_PGSQL_MAX_LINKS+1}" ]; then
		echo "pgsql.max_links=${PHP_FPM_PGSQL_MAX_LINKS}"
	elif [ -n "${PHP_PGSQL_MAX_LINKS+1}" ]; then
		echo "pgsql.max_links=${PHP_PGSQL_MAX_LINKS}"
	fi
	if [ -n "${PHP_FPM_PGSQL_MAX_PERSISTENT+1}" ]; then
	...
	if [ -n "${PHP_FPM_PHALCON_FORM_STRICT_ENTITY_PROPERTY_CHECK+1}" ]; then
		echo "phalcon.form.strict_entity_property_check=${PHP_FPM_PHALCON_FORM_STRICT_ENTITY_PROPERTY_CHECK}"
	...
	if [ -n "${PHP_FPM_PHALCON_ORM_CASE_INSENSITIVE_COLUMN_MAP+1}" ]; then
		echo "phalcon.orm.case_insensitive_column_map=${PHP_FPM_PHALCON_ORM_CASE_INSENSITIVE_COLUMN_MAP}"
	...
	if [ -n "${PHP_FPM_PHALCON_ORM_CAST_LAST_INSERT_ID_TO_INT+1}" ]; then
		echo "phalcon.orm.cast_last_insert_id_to_int=${PHP_FPM_PHALCON_ORM_CAST_LAST_INSERT_ID_TO_INT}"
	...
	if [ -n "${PHP_FPM_PHALCON_ORM_DYNAMIC_UPDATE+1}" ]; then
		echo "phalcon.orm.dynamic_update=${PHP_FPM_PHALCON_ORM_DYNAMIC_UPDATE}"
	...
	if [ -n "${PHP_FPM_PHALCON_ORM_EXCEPTION_ON_FAILED_METADATA_SAVE+1}" ]; then
		echo "phalcon.orm.exception_on_failed_metadata_save=${PHP_FPM_PHALCON_ORM_EXCEPTION_ON_FAILED_METADATA_SAVE}"
	...
	if [ -n "${PHP_FPM_PHALCON_ORM_RESULTSET_PREFETCH_RECORDS+1}" ]; then
		echo "phalcon.orm.resultset_prefetch_records=${PHP_FPM_PHALCON_ORM_RESULTSET_PREFETCH_RECORDS}"
	...
	if [ -n "${PHP_FPM_PHALCON_WARNING_ENABLE+1}" ]; then
		echo "phalcon.warning.enable=${PHP_FPM_PHALCON_WARNING_ENABLE}"
	...
	if [ -n "${PHP_FPM_PHAR_READONLY+1}" ]; then
		echo "phar.readonly=${PHP_FPM_PHAR_READONLY}"
	elif [ -n "${PHP_PHAR_READONLY+1}" ]; then
		echo "phar.readonly=${PHP_PHAR_READONLY}"
	fi
	if [ -n "${PHP_FPM_PHAR_REQUIRE_HASH+1}" ]; then
		echo "phar.require_hash=${PHP_FPM_PHAR_REQUIRE_HASH}"
	elif [ -n "${PHP_PHAR_REQUIRE_HASH+1}" ]; then
		echo "phar.require_hash=${PHP_PHAR_REQUIRE_HASH}"
	fi
	if [ -n "${PHP_FPM_PINBA_AUTO_FLUSH+1}" ]; then
		echo "pinba.auto_flush=${PHP_FPM_PINBA_AUTO_FLUSH}"
	elif [ -n "${PHP_PINBA_AUTO_FLUSH+1}" ]; then
		echo "pinba.auto_flush=${PHP_PINBA_AUTO_FLUSH}"
	fi
	if [ -n "${PHP_FPM_PINBA_ENABLED+1}" ]; then
		echo "pinba.enabled=${PHP_FPM_PINBA_ENABLED}"
	elif [ -n "${PHP_PINBA_ENABLED+1}" ]; then
		echo "pinba.enabled=${PHP_PINBA_ENABLED}"
	fi
	if [ -n "${PHP_FPM_PINBA_RESOLVE_INTERVAL+1}" ]; then
		echo "pinba.resolve_interval=${PHP_FPM_PINBA_RESOLVE_INTERVAL}"
	elif [ -n "${PHP_PINBA_RESOLVE_INTERVAL+1}" ]; then
		echo "pinba.resolve_interval=${PHP_PINBA_RESOLVE_INTERVAL}"
	fi
	if [ -n "${PHP_FPM_PINBA_SERVER+1}" ]; then
		echo "pinba.server=${PHP_FPM_PINBA_SERVER}"
	elif [ -n "${PHP_PINBA_SERVER+1}" ]; then
		echo "pinba.server=${PHP_PINBA_SERVER}"
	fi
	if [ -n "${PHP_FPM_POST_MAX_SIZE+1}" ]; then
		echo "post_max_size=${PHP_FPM_POST_MAX_SIZE}"
	elif [ -n "${PHP_POST_MAX_SIZE+1}" ]; then
		echo "post_max_size=${PHP_POST_MAX_SIZE}"
	fi
	if [ -n "${PHP_FPM_PRECISION+1}" ]; then
		echo "precision=${PHP_FPM_PRECISION}"
	elif [ -n "${PHP_PRECISION+1}" ]; then
		echo "precision=${PHP_PRECISION}"
	fi
	if [ -n "${PHP_FPM_PROTOBUF_KEEP_DESCRIPTOR_POOL_AFTER_REQUEST+1}" ]; then
		echo "protobuf.keep_descriptor_pool_after_request=${PHP_FPM_PROTOBUF_KEEP_DESCRIPTOR_POOL_AFTER_REQUEST}"
	...
	if [ -n "${PHP_FPM_RAPHF_PERSISTENT_HANDLE_LIMIT+1}" ]; then
		echo "raphf.persistent_handle.limit=${PHP_FPM_RAPHF_PERSISTENT_HANDLE_LIMIT}"
	elif [ -n "${PHP_RAPHF_PERSISTENT_HANDLE_LIMIT+1}" ]; then
		echo "raphf.persistent_handle.limit=${PHP_RAPHF_PERSISTENT_HANDLE_LIMIT}"
	fi
	if [ -n "${PHP_FPM_REALPATH_CACHE_SIZE+1}" ]; then
		echo "realpath_cache_size=${PHP_FPM_REALPATH_CACHE_SIZE}"
	elif [ -n "${PHP_REALPATH_CACHE_SIZE+1}" ]; then
		echo "realpath_cache_size=${PHP_REALPATH_CACHE_SIZE}"
	fi
	if [ -n "${PHP_FPM_REALPATH_CACHE_TTL+1}" ]; then
		echo "realpath_cache_ttl=${PHP_FPM_REALPATH_CACHE_TTL}"
	elif [ -n "${PHP_REALPATH_CACHE_TTL+1}" ]; then
		echo "realpath_cache_ttl=${PHP_REALPATH_CACHE_TTL}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_ALGORITHM+1}" ]; then
		echo "redis.arrays.algorithm=${PHP_FPM_REDIS_ARRAYS_ALGORITHM}"
	elif [ -n "${PHP_REDIS_ARRAYS_ALGORITHM+1}" ]; then
		echo "redis.arrays.algorithm=${PHP_REDIS_ARRAYS_ALGORITHM}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_AUTH+1}" ]; then
		echo "redis.arrays.auth=${PHP_FPM_REDIS_ARRAYS_AUTH}"
	elif [ -n "${PHP_REDIS_ARRAYS_AUTH+1}" ]; then
		echo "redis.arrays.auth=${PHP_REDIS_ARRAYS_AUTH}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_AUTOREHASH+1}" ]; then
		echo "redis.arrays.autorehash=${PHP_FPM_REDIS_ARRAYS_AUTOREHASH}"
	elif [ -n "${PHP_REDIS_ARRAYS_AUTOREHASH+1}" ]; then
		echo "redis.arrays.autorehash=${PHP_REDIS_ARRAYS_AUTOREHASH}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_CONNECTTIMEOUT+1}" ]; then
		echo "redis.arrays.connecttimeout=${PHP_FPM_REDIS_ARRAYS_CONNECTTIMEOUT}"
	elif [ -n "${PHP_REDIS_ARRAYS_CONNECTTIMEOUT+1}" ]; then
		echo "redis.arrays.connecttimeout=${PHP_REDIS_ARRAYS_CONNECTTIMEOUT}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_CONSISTENT+1}" ]; then
		echo "redis.arrays.consistent=${PHP_FPM_REDIS_ARRAYS_CONSISTENT}"
	elif [ -n "${PHP_REDIS_ARRAYS_CONSISTENT+1}" ]; then
		echo "redis.arrays.consistent=${PHP_REDIS_ARRAYS_CONSISTENT}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_DISTRIBUTOR+1}" ]; then
		echo "redis.arrays.distributor=${PHP_FPM_REDIS_ARRAYS_DISTRIBUTOR}"
	elif [ -n "${PHP_REDIS_ARRAYS_DISTRIBUTOR+1}" ]; then
		echo "redis.arrays.distributor=${PHP_REDIS_ARRAYS_DISTRIBUTOR}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_FUNCTIONS+1}" ]; then
		echo "redis.arrays.functions=${PHP_FPM_REDIS_ARRAYS_FUNCTIONS}"
	elif [ -n "${PHP_REDIS_ARRAYS_FUNCTIONS+1}" ]; then
		echo "redis.arrays.functions=${PHP_REDIS_ARRAYS_FUNCTIONS}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_HOSTS+1}" ]; then
		echo "redis.arrays.hosts=${PHP_FPM_REDIS_ARRAYS_HOSTS}"
	elif [ -n "${PHP_REDIS_ARRAYS_HOSTS+1}" ]; then
		echo "redis.arrays.hosts=${PHP_REDIS_ARRAYS_HOSTS}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_INDEX+1}" ]; then
		echo "redis.arrays.index=${PHP_FPM_REDIS_ARRAYS_INDEX}"
	elif [ -n "${PHP_REDIS_ARRAYS_INDEX+1}" ]; then
		echo "redis.arrays.index=${PHP_REDIS_ARRAYS_INDEX}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_LAZYCONNECT+1}" ]; then
		echo "redis.arrays.lazyconnect=${PHP_FPM_REDIS_ARRAYS_LAZYCONNECT}"
	elif [ -n "${PHP_REDIS_ARRAYS_LAZYCONNECT+1}" ]; then
		echo "redis.arrays.lazyconnect=${PHP_REDIS_ARRAYS_LAZYCONNECT}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_NAMES+1}" ]; then
		echo "redis.arrays.names=${PHP_FPM_REDIS_ARRAYS_NAMES}"
	elif [ -n "${PHP_REDIS_ARRAYS_NAMES+1}" ]; then
		echo "redis.arrays.names=${PHP_REDIS_ARRAYS_NAMES}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_PCONNECT+1}" ]; then
		echo "redis.arrays.pconnect=${PHP_FPM_REDIS_ARRAYS_PCONNECT}"
	elif [ -n "${PHP_REDIS_ARRAYS_PCONNECT+1}" ]; then
		echo "redis.arrays.pconnect=${PHP_REDIS_ARRAYS_PCONNECT}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_PREVIOUS+1}" ]; then
		echo "redis.arrays.previous=${PHP_FPM_REDIS_ARRAYS_PREVIOUS}"
	elif [ -n "${PHP_REDIS_ARRAYS_PREVIOUS+1}" ]; then
		echo "redis.arrays.previous=${PHP_REDIS_ARRAYS_PREVIOUS}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_READTIMEOUT+1}" ]; then
		echo "redis.arrays.readtimeout=${PHP_FPM_REDIS_ARRAYS_READTIMEOUT}"
	elif [ -n "${PHP_REDIS_ARRAYS_READTIMEOUT+1}" ]; then
		echo "redis.arrays.readtimeout=${PHP_REDIS_ARRAYS_READTIMEOUT}"
	fi
	if [ -n "${PHP_FPM_REDIS_ARRAYS_RETRYINTERVAL+1}" ]; then
		echo "redis.arrays.retryinterval=${PHP_FPM_REDIS_ARRAYS_RETRYINTERVAL}"
	elif [ -n "${PHP_REDIS_ARRAYS_RETRYINTERVAL+1}" ]; then
		echo "redis.arrays.retryinterval=${PHP_REDIS_ARRAYS_RETRYINTERVAL}"
	fi
	if [ -n "${PHP_FPM_REDIS_CLUSTERS_AUTH+1}" ]; then
		echo "redis.clusters.auth=${PHP_FPM_REDIS_CLUSTERS_AUTH}"
	...
	if [ -n "${PHP_FPM_REDIS_CLUSTERS_READ_TIMEOUT+1}" ]; then
		echo "redis.clusters.read_timeout=${PHP_FPM_REDIS_CLUSTERS_READ_TIMEOUT}"
	elif [ -n "${PHP_REDIS_CLUSTERS_READ_TIMEOUT+1}" ]; then
		echo "redis.clusters.read_timeout=${PHP_REDIS_CLUSTERS_READ_TIMEOUT}"
	fi
	if [ -n "${PHP_FPM_REDIS_CLUSTERS_SEEDS+1}" ]; then
		echo "redis.clusters.seeds=${PHP_FPM_REDIS_CLUSTERS_SEEDS}"
	elif [ -n "${PHP_REDIS_CLUSTERS_SEEDS+1}" ]; then
		echo "redis.clusters.seeds=${PHP_REDIS_CLUSTERS_SEEDS}"
	fi
	if [ -n "${PHP_FPM_REDIS_CLUSTERS_TIMEOUT+1}" ]; then
		echo "redis.clusters.timeout=${PHP_FPM_REDIS_CLUSTERS_TIMEOUT}"
	elif [ -n "${PHP_REDIS_CLUSTERS_TIMEOUT+1}" ]; then
		echo "redis.clusters.timeout=${PHP_REDIS_CLUSTERS_TIMEOUT}"
	fi
	if [ -n "${PHP_FPM_REDIS_PCONNECT_CONNECTION_LIMIT+1}" ]; then
	...
	if [ -n "${PHP_FPM_REDIS_SESSION_LOCK_FAILURE_READONLY+1}" ]; then
		echo "redis.session.lock_failure_readonly=${PHP_FPM_REDIS_SESSION_LOCK_FAILURE_READONLY}"
	elif [ -n "${PHP_REDIS_SESSION_LOCK_FAILURE_READONLY+1}" ]; then
		echo "redis.session.lock_failure_readonly=${PHP_REDIS_SESSION_LOCK_FAILURE_READONLY}"
	fi
	if [ -n "${PHP_FPM_REDIS_SESSION_LOCK_RETRIES+1}" ]; then
		echo "redis.session.lock_retries=${PHP_FPM_REDIS_SESSION_LOCK_RETRIES}"
	elif [ -n "${PHP_REDIS_SESSION_LOCK_RETRIES+1}" ]; then
		echo "redis.session.lock_retries=${PHP_REDIS_SESSION_LOCK_RETRIES}"
	fi
	if [ -n "${PHP_FPM_REDIS_SESSION_LOCK_WAIT_TIME+1}" ]; then
		echo "redis.session.lock_wait_time=${PHP_FPM_REDIS_SESSION_LOCK_WAIT_TIME}"
	elif [ -n "${PHP_REDIS_SESSION_LOCK_WAIT_TIME+1}" ]; then
		echo "redis.session.lock_wait_time=${PHP_REDIS_SESSION_LOCK_WAIT_TIME}"
	fi
	if [ -n "${PHP_FPM_REDIS_SESSION_LOCKING_ENABLED+1}" ]; then
		echo "redis.session.locking_enabled=${PHP_FPM_REDIS_SESSION_LOCKING_ENABLED}"
	elif [ -n "${PHP_REDIS_SESSION_LOCKING_ENABLED+1}" ]; then
		echo "redis.session.locking_enabled=${PHP_REDIS_SESSION_LOCKING_ENABLED}"
	fi
	if [ -n "${PHP_FPM_REGISTER_ARGC_ARGV+1}" ]; then
		echo "register_argc_argv=${PHP_FPM_REGISTER_ARGC_ARGV}"
	elif [ -n "${PHP_REGISTER_ARGC_ARGV+1}" ]; then
		echo "register_argc_argv=${PHP_REGISTER_ARGC_ARGV}"
	fi
	if [ -n "${PHP_FPM_REPORT_MEMLEAKS+1}" ]; then
		echo "report_memleaks=${PHP_FPM_REPORT_MEMLEAKS}"
	elif [ -n "${PHP_REPORT_MEMLEAKS+1}" ]; then
		echo "report_memleaks=${PHP_REPORT_MEMLEAKS}"
	fi
	if [ -n "${PHP_FPM_REPORT_ZEND_DEBUG+1}" ]; then
		echo "report_zend_debug=${PHP_FPM_REPORT_ZEND_DEBUG}"
	elif [ -n "${PHP_REPORT_ZEND_DEBUG+1}" ]; then
		echo "report_zend_debug=${PHP_REPORT_ZEND_DEBUG}"
	fi
	if [ -n "${PHP_FPM_REQUEST_ORDER+1}" ]; then
		echo "request_order=${PHP_FPM_REQUEST_ORDER}"
	elif [ -n "${PHP_REQUEST_ORDER+1}" ]; then
		echo "request_order=${PHP_REQUEST_ORDER}"
	fi
	if [ -n "${PHP_FPM_SENDMAIL_FROM+1}" ]; then
		echo "sendmail_from=${PHP_FPM_SENDMAIL_FROM}"
	elif [ -n "${PHP_SENDMAIL_FROM+1}" ]; then
		echo "sendmail_from=${PHP_SENDMAIL_FROM}"
	fi
	if [ -n "${PHP_FPM_SENDMAIL_PATH+1}" ]; then
		echo "sendmail_path=${PHP_FPM_SENDMAIL_PATH}"
	elif [ -n "${PHP_SENDMAIL_PATH+1}" ]; then
		echo "sendmail_path=${PHP_SENDMAIL_PATH}"
	fi
	if [ -n "${PHP_FPM_SERIALIZE_PRECISION+1}" ]; then
		echo "serialize_precision=${PHP_FPM_SERIALIZE_PRECISION}"
	elif [ -n "${PHP_SERIALIZE_PRECISION+1}" ]; then
		echo "serialize_precision=${PHP_SERIALIZE_PRECISION}"
	fi
	if [ -n "${PHP_FPM_SESSION_AUTO_START+1}" ]; then
		echo "session.auto_start=${PHP_FPM_SESSION_AUTO_START}"
	elif [ -n "${PHP_SESSION_AUTO_START+1}" ]; then
		echo "session.auto_start=${PHP_SESSION_AUTO_START}"
	fi
	if [ -n "${PHP_FPM_SESSION_CACHE_EXPIRE+1}" ]; then
		echo "session.cache_expire=${PHP_FPM_SESSION_CACHE_EXPIRE}"
	elif [ -n "${PHP_SESSION_CACHE_EXPIRE+1}" ]; then
		echo "session.cache_expire=${PHP_SESSION_CACHE_EXPIRE}"
	fi
	if [ -n "${PHP_FPM_SESSION_CACHE_LIMITER+1}" ]; then
		echo "session.cache_limiter=${PHP_FPM_SESSION_CACHE_LIMITER}"
	elif [ -n "${PHP_SESSION_CACHE_LIMITER+1}" ]; then
		echo "session.cache_limiter=${PHP_SESSION_CACHE_LIMITER}"
	fi
	if [ -n "${PHP_FPM_SESSION_COOKIE_DOMAIN+1}" ]; then
		echo "session.cookie_domain=${PHP_FPM_SESSION_COOKIE_DOMAIN}"
	elif [ -n "${PHP_SESSION_COOKIE_DOMAIN+1}" ]; then
		echo "session.cookie_domain=${PHP_SESSION_COOKIE_DOMAIN}"
	fi
	if [ -n "${PHP_FPM_SESSION_COOKIE_HTTPONLY+1}" ]; then
		echo "session.cookie_httponly=${PHP_FPM_SESSION_COOKIE_HTTPONLY}"
	elif [ -n "${PHP_SESSION_COOKIE_HTTPONLY+1}" ]; then
		echo "session.cookie_httponly=${PHP_SESSION_COOKIE_HTTPONLY}"
	fi
	if [ -n "${PHP_FPM_SESSION_COOKIE_LIFETIME+1}" ]; then
		echo "session.cookie_lifetime=${PHP_FPM_SESSION_COOKIE_LIFETIME}"
	elif [ -n "${PHP_SESSION_COOKIE_LIFETIME+1}" ]; then
		echo "session.cookie_lifetime=${PHP_SESSION_COOKIE_LIFETIME}"
	fi
	if [ -n "${PHP_FPM_SESSION_COOKIE_PATH+1}" ]; then
		echo "session.cookie_path=${PHP_FPM_SESSION_COOKIE_PATH}"
	...
	if [ -n "${PHP_FPM_SESSION_GC_MAXLIFETIME+1}" ]; then
		echo "session.gc_maxlifetime=${PHP_FPM_SESSION_GC_MAXLIFETIME}"
	elif [ -n "${PHP_SESSION_GC_MAXLIFETIME+1}" ]; then
		echo "session.gc_maxlifetime=${PHP_SESSION_GC_MAXLIFETIME}"
	fi
	if [ -n "${PHP_FPM_SESSION_GC_PROBABILITY+1}" ]; then
		echo "session.gc_probability=${PHP_FPM_SESSION_GC_PROBABILITY}"
	...
	if [ -n "${PHP_FPM_SESSION_REFERER_CHECK+1}" ]; then
		echo "session.referer_check=${PHP_FPM_SESSION_REFERER_CHECK}"
	elif [ -n "${PHP_SESSION_REFERER_CHECK+1}" ]; then
		echo "session.referer_check=${PHP_SESSION_REFERER_CHECK}"
	fi
	if [ -n "${PHP_FPM_SESSION_SAVE_HANDLER+1}" ]; then
		echo "session.save_handler=${PHP_FPM_SESSION_SAVE_HANDLER}"
	elif [ -n "${PHP_SESSION_SAVE_HANDLER+1}" ]; then
		echo "session.save_handler=${PHP_SESSION_SAVE_HANDLER}"
	fi
	if [ -n "${PHP_FPM_SESSION_SAVE_PATH+1}" ]; then
		echo "session.save_path=${PHP_FPM_SESSION_SAVE_PATH}"
	elif [ -n "${PHP_SESSION_SAVE_PATH+1}" ]; then
		echo "session.save_path=${PHP_SESSION_SAVE_PATH}"
	fi
	if [ -n "${PHP_FPM_SESSION_SERIALIZE_HANDLER+1}" ]; then
	...
	if [ -n "${PHP_FPM_SESSION_UPLOAD_PROGRESS_ENABLED+1}" ]; then
		echo "session.upload_progress.enabled=${PHP_FPM_SESSION_UPLOAD_PROGRESS_ENABLED}"
	...
	if [ -n "${PHP_FPM_SESSION_UPLOAD_PROGRESS_FREQ+1}" ]; then
		echo "session.upload_progress.freq=${PHP_FPM_SESSION_UPLOAD_PROGRESS_FREQ}"
	elif [ -n "${PHP_SESSION_UPLOAD_PROGRESS_FREQ+1}" ]; then
		echo "session.upload_progress.freq=${PHP_SESSION_UPLOAD_PROGRESS_FREQ}"
	fi
	if [ -n "${PHP_FPM_SESSION_UPLOAD_PROGRESS_MIN_FREQ+1}" ]; then
		echo "session.upload_progress.min_freq=${PHP_FPM_SESSION_UPLOAD_PROGRESS_MIN_FREQ}"
	...
	if [ -n "${PHP_FPM_SESSION_UPLOAD_PROGRESS_NAME+1}" ]; then
		echo "session.upload_progress.name=${PHP_FPM_SESSION_UPLOAD_PROGRESS_NAME}"
	elif [ -n "${PHP_SESSION_UPLOAD_PROGRESS_NAME+1}" ]; then
		echo "session.upload_progress.name=${PHP_SESSION_UPLOAD_PROGRESS_NAME}"
	fi
	if [ -n "${PHP_FPM_SESSION_UPLOAD_PROGRESS_PREFIX+1}" ]; then
		echo "session.upload_progress.prefix=${PHP_FPM_SESSION_UPLOAD_PROGRESS_PREFIX}"
	elif [ -n "${PHP_SESSION_UPLOAD_PROGRESS_PREFIX+1}" ]; then
		echo "session.upload_progress.prefix=${PHP_SESSION_UPLOAD_PROGRESS_PREFIX}"
	fi
	if [ -n "${PHP_FPM_SESSION_USE_COOKIES+1}" ]; then
		echo "session.use_cookies=${PHP_FPM_SESSION_USE_COOKIES}"
	elif [ -n "${PHP_SESSION_USE_COOKIES+1}" ]; then
		echo "session.use_cookies=${PHP_SESSION_USE_COOKIES}"
	fi
	if [ -n "${PHP_FPM_SESSION_USE_ONLY_COOKIES+1}" ]; then
		echo "session.use_only_cookies=${PHP_FPM_SESSION_USE_ONLY_COOKIES}"
	elif [ -n "${PHP_SESSION_USE_ONLY_COOKIES+1}" ]; then
		echo "session.use_only_cookies=${PHP_SESSION_USE_ONLY_COOKIES}"
	fi
	if [ -n "${PHP_FPM_SESSION_USE_STRICT_MODE+1}" ]; then
		echo "session.use_strict_mode=${PHP_FPM_SESSION_USE_STRICT_MODE}"
	elif [ -n "${PHP_SESSION_USE_STRICT_MODE+1}" ]; then
		echo "session.use_strict_mode=${PHP_SESSION_USE_STRICT_MODE}"
	fi
	if [ -n "${PHP_FPM_SESSION_USE_TRANS_SID+1}" ]; then
		echo "session.use_trans_sid=${PHP_FPM_SESSION_USE_TRANS_SID}"
	elif [ -n "${PHP_SESSION_USE_TRANS_SID+1}" ]; then
		echo "session.use_trans_sid=${PHP_SESSION_USE_TRANS_SID}"
	fi
	if [ -n "${PHP_FPM_SHORT_OPEN_TAG+1}" ]; then
		echo "short_open_tag=${PHP_FPM_SHORT_OPEN_TAG}"
	elif [ -n "${PHP_SHORT_OPEN_TAG+1}" ]; then
		echo "short_open_tag=${PHP_SHORT_OPEN_TAG}"
	fi
	if [ -n "${PHP_FPM_SMTP+1}" ]; then
		echo "SMTP=${PHP_FPM_SMTP}"
	elif [ -n "${PHP_SMTP+1}" ]; then
		echo "SMTP=${PHP_SMTP}"
	fi
	if [ -n "${PHP_FPM_SMTP_PORT+1}" ]; then
		echo "smtp_port=${PHP_FPM_SMTP_PORT}"
	elif [ -n "${PHP_SMTP_PORT+1}" ]; then
		echo "smtp_port=${PHP_SMTP_PORT}"
	fi
	if [ -n "${PHP_FPM_SOAP_WSDL_CACHE+1}" ]; then
		echo "soap.wsdl_cache=${PHP_FPM_SOAP_WSDL_CACHE}"
	elif [ -n "${PHP_SOAP_WSDL_CACHE+1}" ]; then
		echo "soap.wsdl_cache=${PHP_SOAP_WSDL_CACHE}"
	fi
	if [ -n "${PHP_FPM_SOAP_WSDL_CACHE_DIR+1}" ]; then
		echo "soap.wsdl_cache_dir=${PHP_FPM_SOAP_WSDL_CACHE_DIR}"
	elif [ -n "${PHP_SOAP_WSDL_CACHE_DIR+1}" ]; then
		echo "soap.wsdl_cache_dir=${PHP_SOAP_WSDL_CACHE_DIR}"
	fi
	if [ -n "${PHP_FPM_SOAP_WSDL_CACHE_ENABLED+1}" ]; then
		echo "soap.wsdl_cache_enabled=${PHP_FPM_SOAP_WSDL_CACHE_ENABLED}"
	elif [ -n "${PHP_SOAP_WSDL_CACHE_ENABLED+1}" ]; then
		echo "soap.wsdl_cache_enabled=${PHP_SOAP_WSDL_CACHE_ENABLED}"
	fi
	if [ -n "${PHP_FPM_SOAP_WSDL_CACHE_LIMIT+1}" ]; then
		echo "soap.wsdl_cache_limit=${PHP_FPM_SOAP_WSDL_CACHE_LIMIT}"
	elif [ -n "${PHP_SOAP_WSDL_CACHE_LIMIT+1}" ]; then
		echo "soap.wsdl_cache_limit=${PHP_SOAP_WSDL_CACHE_LIMIT}"
	fi
	if [ -n "${PHP_FPM_SOAP_WSDL_CACHE_TTL+1}" ]; then
		echo "soap.wsdl_cache_ttl=${PHP_FPM_SOAP_WSDL_CACHE_TTL}"
	...
	if [ -n "${PHP_FPM_SQLITE3_EXTENSION_DIR+1}" ]; then
		echo "sqlite3.extension_dir=${PHP_FPM_SQLITE3_EXTENSION_DIR}"
	...
	if [ -n "${PHP_FPM_SWOOLE_BLOCKING_DETECTION+1}" ]; then
		echo "swoole.blocking_detection=${PHP_FPM_SWOOLE_BLOCKING_DETECTION}"
	elif [ -n "${PHP_SWOOLE_BLOCKING_DETECTION+1}" ]; then
		echo "swoole.blocking_detection=${PHP_SWOOLE_BLOCKING_DETECTION}"
	fi
	if [ -n "${PHP_FPM_SWOOLE_BLOCKING_THRESHOLD+1}" ]; then
		echo "swoole.blocking_threshold=${PHP_FPM_SWOOLE_BLOCKING_THRESHOLD}"
	elif [ -n "${PHP_SWOOLE_BLOCKING_THRESHOLD+1}" ]; then
		echo "swoole.blocking_threshold=${PHP_SWOOLE_BLOCKING_THRESHOLD}"
	fi
	...
	if [ -n "${PHP_FPM_SWOOLE_LEAK_DETECTION+1}" ]; then
		echo "swoole.leak_detection=${PHP_FPM_SWOOLE_LEAK_DETECTION}"
	elif [ -n "${PHP_SWOOLE_LEAK_DETECTION+1}" ]; then
		echo "swoole.leak_detection=${PHP_SWOOLE_LEAK_DETECTION}"
	fi
	if [ -n "${PHP_FPM_SWOOLE_PROFILE+1}" ]; then
		echo "swoole.profile=${PHP_FPM_SWOOLE_PROFILE}"
	elif [ -n "${PHP_SWOOLE_PROFILE+1}" ]; then
		echo "swoole.profile=${PHP_SWOOLE_PROFILE}"
	fi
	if [ -n "${PHP_FPM_SWOOLE_SOCKET_BUFFER_SIZE+1}" ]; then
		echo "swoole.socket_buffer_size=${PHP_FPM_SWOOLE_SOCKET_BUFFER_SIZE}"
	elif [ -n "${PHP_SWOOLE_SOCKET_BUFFER_SIZE+1}" ]; then
		echo "swoole.socket_buffer_size=${PHP_SWOOLE_SOCKET_BUFFER_SIZE}"
	fi
	...
	if [ -n "${PHP_FPM_TIDY_DEFAULT_CONFIG+1}" ]; then
		echo "tidy.default_config=${PHP_FPM_TIDY_DEFAULT_CONFIG}"
	...
	if [ -n "${PHP_FPM_UPLOAD_TMP_DIR+1}" ]; then
		echo "upload_tmp_dir=${PHP_FPM_UPLOAD_TMP_DIR}"
	...
	if [ -n "${PHP_FPM_USER_AGENT+1}" ]; then
		echo "user_agent=${PHP_FPM_USER_AGENT}"
	elif [ -n "${PHP_USER_AGENT+1}" ]; then
		echo "user_agent=${PHP_USER_AGENT}"
	fi
	if [ -n "${PHP_FPM_USER_DIR+1}" ]; then
		echo "user_dir=${PHP_FPM_USER_DIR}"
	elif [ -n "${PHP_USER_DIR+1}" ]; then
		echo "user_dir=${PHP_USER_DIR}"
	fi
	if [ -n "${PHP_FPM_USER_INI_CACHE_TTL+1}" ]; then
		echo "user_ini.cache_ttl=${PHP_FPM_USER_INI_CACHE_TTL}"
	elif [ -n "${PHP_USER_INI_CACHE_TTL+1}" ]; then
		echo "user_ini.cache_ttl=${PHP_USER_INI_CACHE_TTL}"
	fi
	if [ -n "${PHP_FPM_USER_INI_FILENAME+1}" ]; then
		echo "user_ini.filename=${PHP_FPM_USER_INI_FILENAME}"
	elif [ -n "${PHP_USER_INI_FILENAME+1}" ]; then
		echo "user_ini.filename=${PHP_USER_INI_FILENAME}"
	fi
	if [ -n "${PHP_FPM_VARIABLES_ORDER+1}" ]; then
		echo "variables_order=${PHP_FPM_VARIABLES_ORDER}"
	...
	if [ -n "${PHP_FPM_XDEBUG_CLI_COLOR+1}" ]; then
	...
	if [ -n "${PHP_FPM_XDEBUG_COLLECT_INCLUDES+1}" ]; then
		echo "xdebug.collect_includes=${PHP_FPM_XDEBUG_COLLECT_INCLUDES}"
	elif [ -n "${PHP_XDEBUG_COLLECT_INCLUDES+1}" ]; then
		echo "xdebug.collect_includes=${PHP_XDEBUG_COLLECT_INCLUDES}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_COLLECT_PARAMS+1}" ]; then
		echo "xdebug.collect_params=${PHP_FPM_XDEBUG_COLLECT_PARAMS}"
	elif [ -n "${PHP_XDEBUG_COLLECT_PARAMS+1}" ]; then
		echo "xdebug.collect_params=${PHP_XDEBUG_COLLECT_PARAMS}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_COLLECT_RETURN+1}" ]; then
		echo "xdebug.collect_return=${PHP_FPM_XDEBUG_COLLECT_RETURN}"
	elif [ -n "${PHP_XDEBUG_COLLECT_RETURN+1}" ]; then
		echo "xdebug.collect_return=${PHP_XDEBUG_COLLECT_RETURN}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_COLLECT_VARS+1}" ]; then
		echo "xdebug.collect_vars=${PHP_FPM_XDEBUG_COLLECT_VARS}"
	...
	if [ -n "${PHP_FPM_XDEBUG_DEFAULT_ENABLE+1}" ]; then
	...
	if [ -n "${PHP_FPM_XDEBUG_DUMP_ENV+1}" ]; then
		echo "xdebug.dump.ENV=${PHP_FPM_XDEBUG_DUMP_ENV}"
	elif [ -n "${PHP_XDEBUG_DUMP_ENV+1}" ]; then
		echo "xdebug.dump.ENV=${PHP_XDEBUG_DUMP_ENV}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_DUMP_FILES+1}" ]; then
		echo "xdebug.dump.FILES=${PHP_FPM_XDEBUG_DUMP_FILES}"
	elif [ -n "${PHP_XDEBUG_DUMP_FILES+1}" ]; then
		echo "xdebug.dump.FILES=${PHP_XDEBUG_DUMP_FILES}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_DUMP_GET+1}" ]; then
		echo "xdebug.dump.GET=${PHP_FPM_XDEBUG_DUMP_GET}"
	elif [ -n "${PHP_XDEBUG_DUMP_GET+1}" ]; then
		echo "xdebug.dump.GET=${PHP_XDEBUG_DUMP_GET}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_DUMP_POST+1}" ]; then
		echo "xdebug.dump.POST=${PHP_FPM_XDEBUG_DUMP_POST}"
	elif [ -n "${PHP_XDEBUG_DUMP_POST+1}" ]; then
		echo "xdebug.dump.POST=${PHP_XDEBUG_DUMP_POST}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_DUMP_REQUEST+1}" ]; then
		echo "xdebug.dump.REQUEST=${PHP_FPM_XDEBUG_DUMP_REQUEST}"
	elif [ -n "${PHP_XDEBUG_DUMP_REQUEST+1}" ]; then
		echo "xdebug.dump.REQUEST=${PHP_XDEBUG_DUMP_REQUEST}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_DUMP_SERVER+1}" ]; then
		echo "xdebug.dump.SERVER=${PHP_FPM_XDEBUG_DUMP_SERVER}"
	elif [ -n "${PHP_XDEBUG_DUMP_SERVER+1}" ]; then
		echo "xdebug.dump.SERVER=${PHP_XDEBUG_DUMP_SERVER}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_DUMP_SESSION+1}" ]; then
		echo "xdebug.dump.SESSION=${PHP_FPM_XDEBUG_DUMP_SESSION}"
	elif [ -n "${PHP_XDEBUG_DUMP_SESSION+1}" ]; then
		echo "xdebug.dump.SESSION=${PHP_XDEBUG_DUMP_SESSION}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_DUMP_GLOBALS+1}" ]; then
		echo "xdebug.dump_globals=${PHP_FPM_XDEBUG_DUMP_GLOBALS}"
	elif [ -n "${PHP_XDEBUG_DUMP_GLOBALS+1}" ]; then
		echo "xdebug.dump_globals=${PHP_XDEBUG_DUMP_GLOBALS}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_DUMP_ONCE+1}" ]; then
		echo "xdebug.dump_once=${PHP_FPM_XDEBUG_DUMP_ONCE}"
	elif [ -n "${PHP_XDEBUG_DUMP_ONCE+1}" ]; then
		echo "xdebug.dump_once=${PHP_XDEBUG_DUMP_ONCE}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_DUMP_UNDEFINED+1}" ]; then
		echo "xdebug.dump_undefined=${PHP_FPM_XDEBUG_DUMP_UNDEFINED}"
	...
	if [ -n "${PHP_FPM_XDEBUG_FORCE_ERROR_REPORTING+1}" ]; then
		echo "xdebug.force_error_reporting=${PHP_FPM_XDEBUG_FORCE_ERROR_REPORTING}"
	...
	if [ -n "${PHP_FPM_XDEBUG_IDEKEY+1}" ]; then
	...
	if [ -n "${PHP_FPM_XDEBUG_MAX_STACK_FRAMES+1}" ]; then
		echo "xdebug.max_stack_frames=${PHP_FPM_XDEBUG_MAX_STACK_FRAMES}"
	...
	if [ -n "${PHP_FPM_XDEBUG_PATH_MAPPING+1}" ]; then
		echo "xdebug.path_mapping=${PHP_FPM_XDEBUG_PATH_MAPPING}"
	elif [ -n "${PHP_XDEBUG_PATH_MAPPING+1}" ]; then
		echo "xdebug.path_mapping=${PHP_XDEBUG_PATH_MAPPING}"
	fi
	...
	if [ -n "${PHP_FPM_XDEBUG_PROFILER_ENABLE+1}" ]; then
		echo "xdebug.profiler_enable=${PHP_FPM_XDEBUG_PROFILER_ENABLE}"
	elif [ -n "${PHP_XDEBUG_PROFILER_ENABLE+1}" ]; then
		echo "xdebug.profiler_enable=${PHP_XDEBUG_PROFILER_ENABLE}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_PROFILER_ENABLE_TRIGGER+1}" ]; then
		echo "xdebug.profiler_enable_trigger=${PHP_FPM_XDEBUG_PROFILER_ENABLE_TRIGGER}"
	elif [ -n "${PHP_XDEBUG_PROFILER_ENABLE_TRIGGER+1}" ]; then
		echo "xdebug.profiler_enable_trigger=${PHP_XDEBUG_PROFILER_ENABLE_TRIGGER}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_PROFILER_ENABLE_TRIGGER_VALUE+1}" ]; then
		echo "xdebug.profiler_enable_trigger_value=${PHP_FPM_XDEBUG_PROFILER_ENABLE_TRIGGER_VALUE}"
	...
	if [ -n "${PHP_FPM_XDEBUG_PROFILER_OUTPUT_DIR+1}" ]; then
		echo "xdebug.profiler_output_dir=${PHP_FPM_XDEBUG_PROFILER_OUTPUT_DIR}"
	elif [ -n "${PHP_XDEBUG_PROFILER_OUTPUT_DIR+1}" ]; then
		echo "xdebug.profiler_output_dir=${PHP_XDEBUG_PROFILER_OUTPUT_DIR}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_PROFILER_OUTPUT_NAME+1}" ]; then
		echo "xdebug.profiler_output_name=${PHP_FPM_XDEBUG_PROFILER_OUTPUT_NAME}"
	...
	if [ -n "${PHP_FPM_XDEBUG_REMOTE_CONNECT_BACK+1}" ]; then
		echo "xdebug.remote_connect_back=${PHP_FPM_XDEBUG_REMOTE_CONNECT_BACK}"
	...
	if [ -n "${PHP_FPM_XDEBUG_REMOTE_LOG+1}" ]; then
		echo "xdebug.remote_log=${PHP_FPM_XDEBUG_REMOTE_LOG}"
	...
	if [ -n "${PHP_FPM_XDEBUG_REMOTE_PORT+1}" ]; then
	...
	if [ -n "${PHP_FPM_XDEBUG_SHOW_ERROR_TRACE+1}" ]; then
		echo "xdebug.show_error_trace=${PHP_FPM_XDEBUG_SHOW_ERROR_TRACE}"
	elif [ -n "${PHP_XDEBUG_SHOW_ERROR_TRACE+1}" ]; then
		echo "xdebug.show_error_trace=${PHP_XDEBUG_SHOW_ERROR_TRACE}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_SHOW_EXCEPTION_TRACE+1}" ]; then
		echo "xdebug.show_exception_trace=${PHP_FPM_XDEBUG_SHOW_EXCEPTION_TRACE}"
	elif [ -n "${PHP_XDEBUG_SHOW_EXCEPTION_TRACE+1}" ]; then
		echo "xdebug.show_exception_trace=${PHP_XDEBUG_SHOW_EXCEPTION_TRACE}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_SHOW_LOCAL_VARS+1}" ]; then
		echo "xdebug.show_local_vars=${PHP_FPM_XDEBUG_SHOW_LOCAL_VARS}"
	elif [ -n "${PHP_XDEBUG_SHOW_LOCAL_VARS+1}" ]; then
		echo "xdebug.show_local_vars=${PHP_XDEBUG_SHOW_LOCAL_VARS}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_SHOW_MEM_DELTA+1}" ]; then
	...
	if [ -n "${PHP_FPM_XDEBUG_TRACE_ENABLE_TRIGGER_VALUE+1}" ]; then
		echo "xdebug.trace_enable_trigger_value=${PHP_FPM_XDEBUG_TRACE_ENABLE_TRIGGER_VALUE}"
	...
	if [ -n "${PHP_FPM_XDEBUG_TRACE_FORMAT+1}" ]; then
		echo "xdebug.trace_format=${PHP_FPM_XDEBUG_TRACE_FORMAT}"
	elif [ -n "${PHP_XDEBUG_TRACE_FORMAT+1}" ]; then
		echo "xdebug.trace_format=${PHP_XDEBUG_TRACE_FORMAT}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_TRACE_OPTIONS+1}" ]; then
		echo "xdebug.trace_options=${PHP_FPM_XDEBUG_TRACE_OPTIONS}"
	elif [ -n "${PHP_XDEBUG_TRACE_OPTIONS+1}" ]; then
		echo "xdebug.trace_options=${PHP_XDEBUG_TRACE_OPTIONS}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_TRACE_OUTPUT_DIR+1}" ]; then
		echo "xdebug.trace_output_dir=${PHP_FPM_XDEBUG_TRACE_OUTPUT_DIR}"
	elif [ -n "${PHP_XDEBUG_TRACE_OUTPUT_DIR+1}" ]; then
		echo "xdebug.trace_output_dir=${PHP_XDEBUG_TRACE_OUTPUT_DIR}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_TRACE_OUTPUT_NAME+1}" ]; then
		echo "xdebug.trace_output_name=${PHP_FPM_XDEBUG_TRACE_OUTPUT_NAME}"
	...
	if [ -n "${PHP_FPM_XDEBUG_USE_COMPRESSION+1}" ]; then
		echo "xdebug.use_compression=${PHP_FPM_XDEBUG_USE_COMPRESSION}"
	...
	if [ -n "${PHP_FPM_XDEBUG_VAR_DISPLAY_MAX_DATA+1}" ]; then
		echo "xdebug.var_display_max_data=${PHP_FPM_XDEBUG_VAR_DISPLAY_MAX_DATA}"
	elif [ -n "${PHP_XDEBUG_VAR_DISPLAY_MAX_DATA+1}" ]; then
		echo "xdebug.var_display_max_data=${PHP_XDEBUG_VAR_DISPLAY_MAX_DATA}"
	fi
	if [ -n "${PHP_FPM_XDEBUG_VAR_DISPLAY_MAX_DEPTH+1}" ]; then
		echo "xdebug.var_display_max_depth=${PHP_FPM_XDEBUG_VAR_DISPLAY_MAX_DEPTH}"
	...
	if [ -n "${PHP_FPM_XMLRPC_ERRORS+1}" ]; then
	...
	if [ -n "${PHP_FPM_YAC_DEBUG+1}" ]; then
		echo "yac.debug=${PHP_FPM_YAC_DEBUG}"
	elif [ -n "${PHP_YAC_DEBUG+1}" ]; then
		echo "yac.debug=${PHP_YAC_DEBUG}"
	fi
	if [ -n "${PHP_FPM_YAC_ENABLE+1}" ]; then
		echo "yac.enable=${PHP_FPM_YAC_ENABLE}"
	elif [ -n "${PHP_YAC_ENABLE+1}" ]; then
		echo "yac.enable=${PHP_YAC_ENABLE}"
	fi
	if [ -n "${PHP_FPM_YAC_ENABLE_CLI+1}" ]; then
		echo "yac.enable_cli=${PHP_FPM_YAC_ENABLE_CLI}"
	elif [ -n "${PHP_YAC_ENABLE_CLI+1}" ]; then
		echo "yac.enable_cli=${PHP_YAC_ENABLE_CLI}"
	fi
	if [ -n "${PHP_FPM_YAC_KEYS_MEMORY_SIZE+1}" ]; then
	...
	if [ -n "${PHP_FPM_YAML_DECODE_BINARY+1}" ]; then
		echo "yaml.decode_binary=${PHP_FPM_YAML_DECODE_BINARY}"
	elif [ -n "${PHP_YAML_DECODE_BINARY+1}" ]; then
		echo "yaml.decode_binary=${PHP_YAML_DECODE_BINARY}"
	fi
	if [ -n "${PHP_FPM_YAML_DECODE_PHP+1}" ]; then
		echo "yaml.decode_php=${PHP_FPM_YAML_DECODE_PHP}"
	elif [ -n "${PHP_YAML_DECODE_PHP+1}" ]; then
		echo "yaml.decode_php=${PHP_YAML_DECODE_PHP}"
	fi
	if [ -n "${PHP_FPM_YAML_DECODE_TIMESTAMP+1}" ]; then
		echo "yaml.decode_timestamp=${PHP_FPM_YAML_DECODE_TIMESTAMP}"
	elif [ -n "${PHP_YAML_DECODE_TIMESTAMP+1}" ]; then
		echo "yaml.decode_timestamp=${PHP_YAML_DECODE_TIMESTAMP}"
	fi
	if [ -n "${PHP_FPM_YAML_OUTPUT_CANONICAL+1}" ]; then
		echo "yaml.output_canonical=${PHP_FPM_YAML_OUTPUT_CANONICAL}"
	elif [ -n "${PHP_YAML_OUTPUT_CANONICAL+1}" ]; then
		echo "yaml.output_canonical=${PHP_YAML_OUTPUT_CANONICAL}"
	fi
	if [ -n "${PHP_FPM_YAML_OUTPUT_INDENT+1}" ]; then
		echo "yaml.output_indent=${PHP_FPM_YAML_OUTPUT_INDENT}"
	elif [ -n "${PHP_YAML_OUTPUT_INDENT+1}" ]; then
		echo "yaml.output_indent=${PHP_YAML_OUTPUT_INDENT}"
	fi
	if [ -n "${PHP_FPM_YAML_OUTPUT_WIDTH+1}" ]; then
		echo "yaml.output_width=${PHP_FPM_YAML_OUTPUT_WIDTH}"
	...
	if [ -n "${PHP_FPM_ZEND_ENABLE_GC+1}" ]; then
		echo "zend.enable_gc=${PHP_FPM_ZEND_ENABLE_GC}"
	...
	if [ -n "${PHP_FPM_ZEND_MAX_ALLOWED_STACK_SIZE+1}" ]; then
		echo "zend.max_allowed_stack_size=${PHP_FPM_ZEND_MAX_ALLOWED_STACK_SIZE}"
	elif [ -n "${PHP_ZEND_MAX_ALLOWED_STACK_SIZE+1}" ]; then
		echo "zend.max_allowed_stack_size=${PHP_ZEND_MAX_ALLOWED_STACK_SIZE}"
	fi
	...
	if [ -n "${PHP_FPM_ZEND_RESERVED_STACK_SIZE+1}" ]; then
		echo "zend.reserved_stack_size=${PHP_FPM_ZEND_RESERVED_STACK_SIZE}"
	elif [ -n "${PHP_ZEND_RESERVED_STACK_SIZE+1}" ]; then
		echo "zend.reserved_stack_size=${PHP_ZEND_RESERVED_STACK_SIZE}"
	fi
	if [ -n "${PHP_FPM_ZEND_SCRIPT_ENCODING+1}" ]; then
	...
	if [ -n "${PHP_FPM_ZLIB_OUTPUT_COMPRESSION_LEVEL+1}" ]; then
		echo "zlib.output_compression_level=${PHP_FPM_ZLIB_OUTPUT_COMPRESSION_LEVEL}"
	elif [ -n "${PHP_ZLIB_OUTPUT_COMPRESSION_LEVEL+1}" ]; then
		echo "zlib.output_compression_level=${PHP_ZLIB_OUTPUT_COMPRESSION_LEVEL}"
	fi
	if [ -n "${PHP_FPM_ZLIB_OUTPUT_HANDLER+1}" ]; then
...
) | su -p -s /bin/sh -c 'cat > /etc/php/8.3/fpm/conf.d/99-custom.ini'
