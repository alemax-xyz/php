(
	...
	if [ -n "${PHP_CLI_APC_MMAP_HUGEPAGE_SIZE+1}" ]; then
		echo "apc.mmap_hugepage_size=${PHP_CLI_APC_MMAP_HUGEPAGE_SIZE}"
	...
	if [ -n "${PHP_CLI_ERROR_LOG_MODE+1}" ]; then
		echo "error_log_mode=${PHP_CLI_ERROR_LOG_MODE}"
	...
	if [ -n "${PHP_CLI_FATAL_ERROR_BACKTRACES+1}" ]; then
		echo "fatal_error_backtraces=${PHP_CLI_FATAL_ERROR_BACKTRACES}"
	...
	if [ -n "${PHP_CLI_FIBER_STACK_SIZE+1}" ]; then
		echo "fiber.stack_size=${PHP_CLI_FIBER_STACK_SIZE}"
	...
	if [ -n "${PHP_CLI_MAIL_CR_LF_MODE+1}" ]; then
		echo "mail.cr_lf_mode=${PHP_CLI_MAIL_CR_LF_MODE}"
	...
	if [ -n "${PHP_CLI_MAIL_MIXED_LF_AND_CRLF+1}" ]; then
		echo "mail.mixed_lf_and_crlf=${PHP_CLI_MAIL_MIXED_LF_AND_CRLF}"
	...
	if [ -n "${PHP_CLI_MAX_MEMORY_LIMIT+1}" ]; then
		echo "max_memory_limit=${PHP_CLI_MAX_MEMORY_LIMIT}"
	...
	if [ -n "${PHP_CLI_MYSQLI_LOCAL_INFILE_DIRECTORY+1}" ]; then
		echo "mysqli.local_infile_directory=${PHP_CLI_MYSQLI_LOCAL_INFILE_DIRECTORY}"
	...
	if [ -n "${PHP_CLI_OPCACHE_FILE_CACHE_READ_ONLY+1}" ]; then
		echo "opcache.file_cache_read_only=${PHP_CLI_OPCACHE_FILE_CACHE_READ_ONLY}"
	...
	if [ -n "${PHP_CLI_OPCACHE_JIT_MAX_TRACE_LENGTH+1}" ]; then
		echo "opcache.jit_max_trace_length=${PHP_CLI_OPCACHE_JIT_MAX_TRACE_LENGTH}"
	...
	if [ -n "${PHP_CLI_OPENSSL_LIBCTX+1}" ]; then
		echo "openssl.libctx=${PHP_CLI_OPENSSL_LIBCTX}"
	...
	if [ -n "${PHP_CLI_REDIS_SESSION_LOCK_FAILURE_READONLY+1}" ]; then
		echo "redis.session.lock_failure_readonly=${PHP_CLI_REDIS_SESSION_LOCK_FAILURE_READONLY}"
	...
	if [ -n "${PHP_CLI_SESSION_COOKIE_PARTITIONED+1}" ]; then
		echo "session.cookie_partitioned=${PHP_CLI_SESSION_COOKIE_PARTITIONED}"
	...
	if [ -n "${PHP_CLI_SWOOLE_BLOCKING_DETECTION+1}" ]; then
		echo "swoole.blocking_detection=${PHP_CLI_SWOOLE_BLOCKING_DETECTION}"
	elif [ -n "${PHP_SWOOLE_BLOCKING_DETECTION+1}" ]; then
		echo "swoole.blocking_detection=${PHP_SWOOLE_BLOCKING_DETECTION}"
	fi
	if [ -n "${PHP_CLI_SWOOLE_BLOCKING_THRESHOLD+1}" ]; then
		echo "swoole.blocking_threshold=${PHP_CLI_SWOOLE_BLOCKING_THRESHOLD}"
	...
	if [ -n "${PHP_CLI_SWOOLE_LEAK_DETECTION+1}" ]; then
		echo "swoole.leak_detection=${PHP_CLI_SWOOLE_LEAK_DETECTION}"
	elif [ -n "${PHP_SWOOLE_LEAK_DETECTION+1}" ]; then
		echo "swoole.leak_detection=${PHP_SWOOLE_LEAK_DETECTION}"
	fi
	if [ -n "${PHP_CLI_SWOOLE_PROFILE+1}" ]; then
		echo "swoole.profile=${PHP_CLI_SWOOLE_PROFILE}"
	elif [ -n "${PHP_SWOOLE_PROFILE+1}" ]; then
		echo "swoole.profile=${PHP_SWOOLE_PROFILE}"
	fi
	if [ -n "${PHP_CLI_SWOOLE_SOCKET_BUFFER_SIZE+1}" ]; then
		echo "swoole.socket_buffer_size=${PHP_CLI_SWOOLE_SOCKET_BUFFER_SIZE}"
	...
	if [ -n "${PHP_CLI_XDEBUG_PATH_MAPPING+1}" ]; then
		echo "xdebug.path_mapping=${PHP_CLI_XDEBUG_PATH_MAPPING}"
	...
	if [ -n "${PHP_CLI_ZEND_MAX_ALLOWED_STACK_SIZE+1}" ]; then
		echo "zend.max_allowed_stack_size=${PHP_CLI_ZEND_MAX_ALLOWED_STACK_SIZE}"
	...
	if [ -n "${PHP_CLI_ZEND_RESERVED_STACK_SIZE+1}" ]; then
		echo "zend.reserved_stack_size=${PHP_CLI_ZEND_RESERVED_STACK_SIZE}"
...
) | su -p -s /bin/sh -c 'cat > /etc/php/8.5/cli/conf.d/99-custom.ini'
