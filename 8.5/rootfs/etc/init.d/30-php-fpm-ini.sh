(
	...
	if [ -n "${PHP_FPM_FATAL_ERROR_BACKTRACES+1}" ]; then
		echo "fatal_error_backtraces=${PHP_FPM_FATAL_ERROR_BACKTRACES}"
	elif [ -n "${PHP_FATAL_ERROR_BACKTRACES+1}" ]; then
		echo "fatal_error_backtraces=${PHP_FATAL_ERROR_BACKTRACES}"
	fi
	...
	if [ -n "${PHP_FPM_MAIL_CR_LF_MODE+1}" ]; then
		echo "mail.cr_lf_mode=${PHP_FPM_MAIL_CR_LF_MODE}"
	elif [ -n "${PHP_MAIL_CR_LF_MODE+1}" ]; then
		echo "mail.cr_lf_mode=${PHP_MAIL_CR_LF_MODE}"
	fi
	...
	if [ -n "${PHP_FPM_MAX_MEMORY_LIMIT+1}" ]; then
		echo "max_memory_limit=${PHP_FPM_MAX_MEMORY_LIMIT}"
	elif [ -n "${PHP_MAX_MEMORY_LIMIT+1}" ]; then
		echo "max_memory_limit=${PHP_MAX_MEMORY_LIMIT}"
	fi
	...
	if [ -n "${PHP_FPM_OPCACHE_FILE_CACHE_READ_ONLY+1}" ]; then
		echo "opcache.file_cache_read_only=${PHP_FPM_OPCACHE_FILE_CACHE_READ_ONLY}"
	elif [ -n "${PHP_OPCACHE_FILE_CACHE_READ_ONLY+1}" ]; then
		echo "opcache.file_cache_read_only=${PHP_OPCACHE_FILE_CACHE_READ_ONLY}"
	fi
	...
	if [ -n "${PHP_FPM_OPENSSL_LIBCTX+1}" ]; then
		echo "openssl.libctx=${PHP_FPM_OPENSSL_LIBCTX}"
	elif [ -n "${PHP_OPENSSL_LIBCTX+1}" ]; then
		echo "openssl.libctx=${PHP_OPENSSL_LIBCTX}"
	fi
	...
	if [ -n "${PHP_FPM_SESSION_COOKIE_PARTITIONED+1}" ]; then
		echo "session.cookie_partitioned=${PHP_FPM_SESSION_COOKIE_PARTITIONED}"
	elif [ -n "${PHP_SESSION_COOKIE_PARTITIONED+1}" ]; then
		echo "session.cookie_partitioned=${PHP_SESSION_COOKIE_PARTITIONED}"
	fi
...
) | su -p -s /bin/sh -c 'cat > /etc/php/8.5/fpm/conf.d/99-custom.ini'
